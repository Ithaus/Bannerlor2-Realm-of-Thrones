...
## 2026-09-20 (5) - WYJSCIE AWARYJNE Z ROZMOWY samo klikalo pierwsza opcje (u kompana: pojedynek ROT) - teraz wchodzi do opcji przez wiersz NPC, jak vanilla
**Mod:** CrashScribe | **Pliki:** `CrashScribe/src/DialogEscape.cs`
**Zgloszenie (Jeff 20.09):** "podjezdzam pogadac ze swoim kompanem, ktory ma swoja party, nie moge nic zrobic, wlacza sie pojedynkowanie, a ja chce pogadac i wymienic sie wojskiem" / "nie da sie z nim rozmawiac, od razu chce sie pojedynkowac".
**Dowod z logu (`session-2026-09-20_08-20-25.log`):** `[08:22:18]` menu `encounter_meeting` (spotkanie z partia na mapie), potem `[08:27:25] DialogEscape: rozmowa uratowana (1) - opcje z hero_main_options.` - czyli rozmowa stanela w martwym stanie, Jeff kliknal nasze awaryjne "Let us talk", a zaraz potem "samo" ruszylo cos, czego nie wybieral.
**PRZYCZYNA (dekompilacja `TaleWorlds.CampaignSystem.dll`, `ConversationManager`):** po wierszu GRACZA silnik wola `DoOption -> DoOptionContinue -> ProcessPartnerSentence -> GetSentenceOptions(onlyPlayer: false, processAfterOneOption: false)`. Ta metoda bierze PIERWSZY wiersz w stanie wyjsciowym (wg priorytetu), ktorego warunek przechodzi - **takze wiersz gracza** (`GetSentenceMatch` filtruje `IsPlayer` tylko przy `onlyPlayer: true`) - i `ProcessSentence` WYKONUJE go jak klikniety. Nasze "Let us talk" skakalo z wiersza gracza WPROST do `hero_main_options`, czyli do stanu zlozonego z samych opcji gracza - wiec silnik sam "klikal" pierwsza pasujaca. U kompana z wlasna partia (Clan == PlayerClan, obaj zdrowi) byl to wiersz ROT `AddCompanionDuelDialogFlow` ("Let's test our skills one on one", `ROTDuelsBehavior`, priorytet 100) -> "Very well." -> menu pojedynku. Vanilla nigdy nie wchodzi do `hero_main_options` z wiersza gracza: zawsze przez wiersz NPC (`hero_ask_4` "So, then. What is it?" w `lord_start`, "Anything else?" w `lord_pretalk`). Wymiana wojsk z partia klanu to w vanilli opcja `clan_member_manage_troops` "Let me inspect your troops" w `hero_main_options` - byla tam caly czas, tylko nikt nie zdazyl jej zobaczyc.
**Zmiana:** nowy wiersz NPC `cs_escape_npc`: `cs_escape_pretalk -> hero_main_options` ("So, then. What is it?", bez warunku, priorytet 100), a wszystkie sześć "Let us talk" prowadzi teraz do `cs_escape_pretalk` zamiast wprost do `hero_main_options`. "Farewell" bez zmian (close_window to stan koncowy, nie lista opcji).
**Nie ruszone (osobna sprawa):** DLACZEGO rozmowa z kompanem w ogole staje w martwym stanie. Vanilla ma w `lord_start` wiersz `hero_ask_4` bez warunku, wiec `lord_start` nie moze byc martwy - martwy musial byc `lord_introduction` albo `lord_pretalk`, albo ktorys mod (AIInfluence jest zaciemniony i ma `DisableTalkButtonPatch`, `DialogAutoCloseEndConversationPatch`) przechwytuje "start". Log tego nie mowi; jesli po tej zmianie "Let us talk" nadal bedzie potrzebne przy kazdym kompanie, trzeba dolozyc do `Stuck()` wypis aktywnego tokenu i rozmowcy.
**Ryzyko / co sprawdzic:** po "Let us talk" kompan ma powiedziec "So, then. What is it?" i pokazac normalna liste (m.in. "Let me inspect your troops"), pojedynek ma byc JEDNA z opcji, nie automatem. W logu `DialogEscape: rozmowa uratowana (N) - przez wiersz NPC do hero_main_options.` Ta sama poprawka dotyczy kaplanow BK (te same wiersze).
**Status:** WGRANE 2026-09-20 (CrashScribe.dll md5 907af2844b34395a994c9fbe831da505, build rc=0, gra zamknieta; poprzednie DLL obok jako `CrashScribe.dll.bak-2026-09-20-przed-wierszem-npc`). DO SPRAWDZENIA po wczytaniu.


## 2026-09-20 (4) - TWARDY SEN: AI dostawalo w nocy cudze rozkazy i jechalo z namiotem na plecach - teraz straznik kladzie ich z powrotem i loguje, kto budzil
**Mod:** Armoury | **Pliki:** `Armoury/src/NightRest.cs` (HoldSleepers NOWE, _bedPos, raport nocy w AiNightCamp, wywolanie z OnTick)
**Zgloszenie (Jeff 20.09):** "ja musze odpoczywac i rozstawiac namiot i stac w miejscu, a AI i bandy tego nie robia, a jak rozstawiaja namiot, to namiot porusza sie i odpoczywaja i sie przemieszczaja. Kazdy ma miec takie same zasady jak ja i musi spac, oprocz nieumarlych."
**Punkt wyjscia:** obozowanie AI JEST (`AiNightCamp`, 22-4, `AiCampsAtNight = true` w MCM Jeffa): lordowie i karawany dostaja `Ai.DisableForHours(1)` + `SetMoveModeHold()`. Umarli sa wylaczeni (`Undead.Party`). Log Armoury nie mial ANI JEDNEJ linii z tej funkcji (logowala tylko wyjatki), wiec nie bylo jak sprawdzic, czy dziala.
**PRZYCZYNA (sprawdzone w dekompilacji, nie zgadniete):**
1. `MobilePartyAi.DisableForHours` gasi tylko WLASNE myslenie partii (`TickInternal`: przy `IsDisabled` nie wola `GetBehaviors`). NIE blokuje ruchu do celu, ktory ktos nada z zewnatrz przez `MobileParty.SetMove*` - te metody ustawiaja `MoveTargetPoint` wprost i partia jedzie.
2. `StrategicCampaignAI145.dll` (`StrategicCampaignAI145Behavior.OnHourlyTick`) co 4 godziny (`hour % 4 == 0`, czyli TAKZE 0:00 i 4:00 - w srodku nocy) wola na wodzach armii `SetMoveBesiegeSettlement/Defend/Patrol/GoToSettlement/EngageParty` "IfNeeded", gdzie "needed" = `leader.TargetSettlement != target`. Po naszym `SetMoveModeHold()` `TargetSettlement` jest null, wiec warunek jest ZAWSZE spelniony i rozkaz pada za kazdym razem. Ten mod nigdzie nie czyta `Ai.IsDisabled` (binarne przeszukanie: `get_IsDisabled` brak). Wodz armii rusza, eskorta za nim.
3. `AIInfluence.dll` (zaciemniony) tez wola `SetMoveGoToSettlement/EngageParty/Escort/Patrol/Raid/Besiege` i tez nie czyta `IsDisabled`; ma wlaczone `EnableNPCMapInitiative`. Nie do sprawdzenia w kodzie, ale straznik lapie go tak samo.
4. Namiot "porusza sie", bo `RefreshNearbyTents` trzyma namiot na kazdej partii z `Ai.IsDisabled == true` - a taka partia po cudzym rozkazie jedzie z wlaczona flaga snu. Wizerunek namiotu jest dzieckiem wizerunku partii, wiec jedzie razem z nia.
**Zmiana:** (a) `_bedPos` - pozycja z chwili polozenia sie; (b) `HoldSleepers()` z `OnTick` co ~1 s realna w oknie 22-4: kazdy spiacy z `DefaultBehavior != Hold` albo z celem (`TargetSettlement`/`TargetParty`) albo przesuniety > 0.3 od legowiska dostaje ponownie `DisableForHours(1)` + `SetMoveModeHold()`; pierwsze trzy przypadki w sesji w calosci do logu (kto, jaki rozkaz, dokad, o ile zjechal, czy wodz armii), potem co dwudziesty; (c) raport co godzine nocy: `AiNightCamp: H:00 - spi N (lordow, karawan, band, w tym wodzow armii); obudzonych cudza reka od poprzedniej godziny: K`. Zapamietany rozkaz sprzed snu (`RememberOrder`) zostaje nietkniety i wraca o swicie jak dotad; alarm (wrog blisko) dalej zwalnia ze snu i czysci `_bedPos`. Bandyci bez zmian: obozuja tylko za `AiBanditsCampToo` (u Jeffa OFF) i maja wlasne natury dzienne/nocne (`AiBanditRest`).
**Ryzyko / co sprawdzic:** w nocy linie `AiNightCamp: 22:00 - spi ...` co godzine; jesli `obudzonych cudza reka` > 0, to dowod, ze ktos wydaje rozkazy w nocy, a linie `obudzony cudza reka <lord> (rozkaz BesiegeSettlement -> X, wodz armii)` mowia kto (przy SCA beda o 0:00 i 4:00). Namioty maja stac w miejscu. Koszt: petla po `_camping` (kilkaset partii) raz na sekunde realna - kilka odczytow pol na partie, bez wizerunkow. Jesli SCA po tym przestanie prowadzic armie sensownie za dnia (bo co noc tracimy jego cel i o swicie oddajemy STARY rozkaz), rozwazyc oddawanie o swicie OSTATNIEGO cudzego rozkazu zamiast pierwszego - na razie nie ruszam, jedna zmiana naraz.
**Status:** WGRANE 2026-09-20 (Armoury.dll md5 88304e56ddcb91d160386a2e73c62912, build rc=0, gra zamknieta; poprzednie DLL obok jako `Armoury.dll.bak-2026-09-20-przed-twardym-snem`). DO SPRAWDZENIA po wczytaniu - patrz linie `AiNightCamp:` w logu Armoury w nocy.


## 2026-09-20 (3) - POCHOD NOCNEGO KROLA: horda stala nie z braku sil, tylko dlatego, ze ROT nie ma jej czym ruszyc - teraz dostaje cel wprost
**Mod:** CrashScribe | **Pliki:** `CrashScribe/src/NightKingCall.cs` (March, PickTarget, Beyond, IsOthers, Invasion, Resting, Candidates), `CrashScribe/src/Config.cs`, `CrashScribe/ModuleData/CrashScribe.settings.xml`
**Zgloszenie (Jeff 19-20.09):** "popraw AI nocnego krola, ma zaczac zdobywac miasta".
**Punkt wyjscia:** Zew Nocnego Krola (16.09) dziala - log 19.09 siedem razy jednego dnia: `Zew Nocnego Krola: Dryn ma 558 zdrowych trupow (cel 520) - oblezenie w rekach ROT`. Mimo to 0 zdobytych osad przez ~193 dni kampanii.
**PRZYCZYNA (dekompilacja zainstalowanego `ROT-Core/bin/Win64_Shipping_Client/ROT.dll`, 20.09):**
1. **Za Murem jest 7 FORTYFIKACJI, nie 25.** Pozostale 18 to wioski, a wioska nie ma wlasnego wlasciciela (`Village.MapFaction => Bound.MapFaction`) - przechodzi razem ze swoim lennem. Do zapalenia `IsInvasionStarted` trzeba wiec SIEDMIU zdobyczy: Thenn (town_S6), Frostfang's Camp (town_S4), Hornfoot (castle_N6), Hardhome (town_S7), Fist of the First Men (castle_S6), Craster's Keep (castle_S5), Frozen Shore (ROT_castle45). Warunek inwazji JEST osiagalny - to nie tu lezal problem.
2. **Vanilla nie wydaje Innym zadnych rozkazow:** `AiMilitaryBehavior.AiHourlyTick` wychodzi natychmiast przy `!MapFaction.IsKingdomFaction`, a klan Innych (`ROTclan_126`) to minor faction bez krolestwa (`spclans.xml:514`). Jedynym zrodlem rozkazow jest `ROTOthersCampaignBehavior`.
3. **TRZY Z CZTERECH zachowan ROT dla Innych sa strukturalnie niewykonalne.** `ROT.HarmonyPatches.Core.AIThinkPatch` przejmuje `AiPartyThinkBehavior.PartyHourlyAiTick` i przy `!(MapFaction is Kingdom)` zwycieski wynik z `willGatherArmy: true` NIE JEST NADAWANY (najwyzej jednorazowy patrol przy `DefaultBehavior == Hold`). Rajd (l.385, 428), obrona (l.438, 480) i patrol (l.490) maja `willGatherArmy: true`. Tylko OBLEZENIE (l.368, 561) ma `false` - i tylko ono da sie wykonac.
4. **A oblezenie ROT wystawia dopiero przy przewadze 2.5x nad garnizonem z milicja** (l.527 i 558-559). Gdy zaden cel nie przechodzi - `goto IL_0402` i horda spada na rajd/obrone/patrol, czyli na rozkazy, ktore i tak nie zostana nadane. **Banda nie dostaje ZADNEGO rozkazu i stoi.** To jest dokladnie objaw Jeffa: 558 zdrowych, prog 500 spelniony, zero ruchu.
5. Druga bramka (`PartySizeRatio >= 0.8`) jest SPELNIONA dzieki Zewowi - dowod z naszego logu: `"-> 520 zdrowych z 783 ... limit 921"` = 0.850. Blokuje wylacznie przewaga 2.5x: przy hordzie 556-916 ludzi wymaga ona celu z garnizonem <= 222-366, a za Murem sa m.in. trzy duze miasta.
**Zmiana (wariant "nadac cel wprost"):** raz na dzien, po Zewie, gotowa horda (>= `NightKingMarchMin` zdrowych, domyslnie 500) dostaje `SetPartyAiAction.GetActionForBesiegingSettlement` na wybrana osade za Murem. Cel wybierany z WLASNEJ listy ROT (`_settlementsToSiege`), filtrowany przez `ROTUtilities.IsBeyondTheWall` (ten sam wzor, ktorego uzywa ROT), z pominieciem Muru i osad juz nalezacych do Innych; wymagany stan wojny i przewaga >= `NightKingMarchOdds` (domyslnie 2.0, bo prog samego ROT 2.5 nie przepuszcza nic). Przy remisie blizszy cel. Rozkazu NIE ruszamy, jesli banda juz idzie na wlasciwy cel albo jest w bitwie/oblezeniu; szanujemy 5-dniowa przerwe ROT po oblezeniu (`_siegeCooldowns`, ustawiana takze po przegranym szturmie); po zapaleniu `IsInvasionStarted` przestajemy sie wtracac i rzadzi ROT. Po nadaniu rozkazu ROT sam go podtrzymuje (l.355-372 dopisuje temu samemu celowi 999 punktow z `willGatherArmy: false`), a gdy horda spadnie ponizej 500, zwycieskie beda rajd/obrona/patrol - czyli nic, co da sie nadac - wiec rozkaz zostaje i marsz trwa.
**Sprawdzone przed napisaniem kodu:** `SetPartyAiAction.GetActionForBesiegingSettlement(MobileParty, Settlement, MobileParty.NavigationType, bool)` istnieje z ta sygnatura; `ROTUtilities.IsBeyondTheWall(Vec2)` przyjmuje `Vec2`, a `Settlement.GetPosition2D` zwraca `Vec2` - dokladnie tak, jak wola to sam ROT (l.309); pola `_settlementsToSiege` (IEnumerable<Settlement>), `IsInvasionStarted` (internal bool), `_siegeCooldowns` (Dictionary<Hero, CampaignTime>) istnieja pod tymi nazwami.
**Ryzyko / co sprawdzic:** przy starcie `Pochod Nocnego Krola: czynny - od 500 zdrowych ...`. Raz na dzien tabelka `osady za Murem (horda N ludzi, limit L)` z garnizonem, przewaga, dystansem i stanem WOJNA/POKOJ dla kazdej z siedmiu fortyfikacji - z niej od razu widac, czy problemem jest sila, czy brak wojny (`SetupWars` ROT jest wolane TYLKO przy nowej grze, wiec klany powstale pozniej moga nie byc w stanie wojny z Innymi; wtedy osobna zmiana). Potem `RUSZA NA <osada>` i `juz idzie na <osada> (dystans ...)`. Pokretla: `NightKingMarchOdds` w gore, jesli horda ma ginac rzadziej; `NightKingMarchEnabled` false, zeby wylaczyc. Glowne ryzyko: horda faktycznie ruszy i zacznie zdobywac osady za Murem, a po siedmiu zapali sie inwazja na poludnie - to jest cel, ale warto wiedziec, ze od tego momentu ROT zaczyna wlasna fabule.
**Status:** WGRANE 2026-09-20 (CrashScribe.dll md5 22fe1837b8f2716d9f53fee9f63b540a, build rc=0, gra zamknieta; poprzednie DLL obok jako `.bak-2026-09-20-przed-pochodem`; `CrashScribe.settings.xml` podmieniony z piecioma nowymi kluczami, kopia obok, XML sprawdzony parserem). DO SPRAWDZENIA po wczytaniu.


## 2026-09-20 (2) - TARCZA PRZED ZARAZA: czyszczenie danych i suwak MCM ZAWIODLY (udokumentowane), wiec choroba po prostu nie zabija juz ludzi gracza
**Mod:** Armoury | **Pliki:** `Armoury/src/ManLedger.cs` (LossPrefix -> tarcza), `Armoury/src/Settings.cs` (+McmSettings: PlagueSparesYourMen, PlagueShieldLogEvery)
**Zgloszenie (Jeff 20.09):** "nadal trace ludzi, sprawdz czy to co zrobiles zadzialalo".
**Dowod, ze NIE zadzialalo (`Armoury-2026-09-20_04-42-04.log`, sesja po obu zmianach):** `[04:43:39]` i `[04:44:15]` `KSIEGA LUDZI: -1 Riverlands Cavalry, w obozie oblezniczym | zabral: ... <- DiseaseEffectSystem.KillTroopsFromDisease`. Ta sama przyczyna co poprzednio.
**Dlaczego obie proby zawiodly - sprawdzone, nie zgadniete:**
1. WYLECZENIE DANYCH NIE TRZYMA SIE. Po moim czyszczeniu partia gracza miala 35 wpisow i 0 czynnych ognisk. Po jednej sesji gry plik `Modules/AIInfluence/save_data/PEh4X2ByGArW/disease_instances.json` ma mtime 04:44:15 (przepisany przez mod) i **36 wpisow, wszystkie 36 CZYNNE, 21 zarazonych zolnierzy**. Czyli prawdziwy stan chorob mod trzyma w SAVE, a ten JSON jest tylko wypisem, ktory nadpisuje przy zapisie. Edycja pliku przy zamknietej grze jest bezcelowa.
2. SUWAK MCM NIE BRAMKUJE SMIERCI ZOLNIERZY. `DiseaseMaxDeathChance` = **0.0** PRZETRWALO start gry (sprawdzone w `Configs/ModSettings/Global/AIInfluence/AIInfluenceSettings.json`, mtime 04:33:29, wartosc nadal 0.0), a ludzie i tak gina. Ten suwak najwyrazniej dotyczy smierci BOHATEROW, nie zolnierzy. Kodu nie da sie sprawdzic - `AIInfluence.dll` jest zaciemniony (ilspycmd nie czyta typow, `DiseaseEffectSystem` widac tylko w stosie wywolan i w binarnym przeszukaniu DLL).
**Zmiana:** tarcza w `ManLedger.LossPrefix` - czyli w jedynym miejscu, ktore na pewno dziala: `TroopRoster.AddToCountsAtIndex`, przez ktore przechodzi KAZDE zejscie sztuki z rostera. Gdy ubytek dotyczy partii GRACZA i slad stosu zawiera "Disease", prefix ODMAWIA zmiany (`__result = index; return false`) - oryginal nie biegnie i nikt nie ginie. Prefix zmienil typ z void na bool. Chorobie zostaja WSZYSTKIE pozostale kary (morale, predkosc mapy, skille) - tylko przestaje zabijac. Partie AI w ogole tu nie wchodza (warunek `main.MemberRoster != roster`), wiec swiat chowa swoich umarlych jak dotad. Wylacznik `PlagueSparesYourMen` (domyslnie wlaczony) i `PlagueShieldLogEvery` (co ktory zapis do logu po pierwszych trzech) w MCM Armoury.
**Ryzyko / co sprawdzic:** przy starcie `ManLedger: ksiega ludzi czynna ... Tarcza przed zaraza WLACZONA`. Przy kazdej odmowie `TARCZA PRZED ZARAZA: odmowiono smierci 1 x <oddzial> | probowal: ... DiseaseEffectSystem.KillTroopsFromDisease | uratowanych w sesji: N | partia ma dalej M.` Linie `KSIEGA LUDZI: ... DiseaseEffectSystem` maja ZNIKNAC. Dopasowanie po slowie "Disease" w sladzie stosu jest z zalozenia szerokie - zlapie kazdy system chorob, takze przyszly; jesli kiedys trzeba bedzie pozwolic zarazie zabijac, wylaczyc suwakiem. Licznik `infected_troop_count` w danych AIInfluence moze sie rozjechac (mod bedzie uwazal, ze zabil), co jest bez znaczenia, bo i tak nadpisuje te dane.
**Cofniete z poprzedniego wpisu:** czyszczenie `disease_instances.json` zostawiam bez przywracania (mod i tak je nadpisal), `DiseaseMaxDeathChance = 0.0` zostaje - nie szkodzi, a ogranicza smierc bohaterow. Kopie: `disease_instances.json.bak-2026-09-20-przed-czyszczeniem`, `AIInfluenceSettings.json.bak-2026-09-20-przed-smiertelnoscia`.
**Status:** WGRANE 2026-09-20 (Armoury.dll md5 1c69572bc843c947893b96b4a01c304c, build rc=0, gra zamknieta; poprzednie DLL obok jako `Armoury.dll.bak-2026-09-20-przed-tarcza`). DO SPRAWDZENIA po wczytaniu.


## 2026-09-20 - ZNALEZIONY ZLODZIEJ LUDZI: mod AIInfluence i jego system chorob. 35 ognisk "The Dock Fever" naraz w partii gracza - wyleczone, smiertelnosc zbita do zera
**Mod:** cudzy (AIInfluence) - zmiana w jego danych i ustawieniach, nasz kod bez zmian | **Pliki:** `Modules/AIInfluence/save_data/PEh4X2ByGArW/disease_instances.json`, `Configs/ModSettings/Global/AIInfluence/AIInfluenceSettings.json`
**Zgloszenie (Jeff 19-20.09, czwarty raz):** "czemu nadal trace ludzi, jestem w oblezeniu, save rozpoczalem od 318, teraz mam 313".
**JAK ZNALEZIONE:** przez wgrana poprzednio `ManLedger` (ksiega ludzi). Log `Armoury-2026-09-20_03-25-59.log`:
`[04:25:44] KSIEGA LUDZI: -1 Riverlands Cavalry, w obozie oblezniczym | zabral: TroopRoster.AddToCountsAtIndex_Patch2 <- TroopRoster.RemoveTroop_Patch1 <- DiseaseEffectSystem.KillTroopsFromDisease | ta przyczyna dzis: 1 ludzi w 1 ruchach | razem w sesji: 1 | partia ma teraz 318.`
Trzy takie linie w godzine (Riverlands Cavalry x2, Northern Mounted Warlord x1). Binarne przeszukanie wszystkich DLL: `KillTroopsFromDisease` i `DiseaseEffectSystem` wystepuja WYLACZNIE w `AIInfluence/bin/Win64_Shipping_Client/AIInfluence.dll` - to ten mod, ktory jest zaciemniony (Confuser) i przez to nie pokazywal sie we wczesniejszych przeszukiwaniach po typach. To zamyka sledztwo ciagnace sie od 19.09: wczesniejsze podejrzenia (bitwa, nasze prawo zaleglego zoldu, CampFever, straty RealisticBannerlord) byly kolejno wykluczane, ale zawsze zostawala "reszta", ktorej nic nie logowalo.
**CO SIEDZIALO W OBOZIE (dane moda, `disease_instances.json`, 2062 wpisy, 800 czynnych w calym swiecie):** partia gracza miala **35 CZYNNYCH ognisk "The Dock Fever" naraz, wszystkie NIELECZONE**, w tym 15 na 100% rozwoju, lacznie 17 zarazonych zolnierzy. Wlasne ustawienia tego moda mowia `DiseaseMaxSimultaneous = 3` i `DiseaseMinDaysBetweenOutbreaks = 14`, wiec 35 ognisk TEJ SAMEJ choroby na jednej partii to kumulacja wbrew jego wlasnym regulom (wyglada na blad AIInfluence; kod zaciemniony, nie do sprawdzenia). Kazde ognisko zabija niezaleznie, przy `DiseaseMaxDeathChance = 0.3` - stad staly, niewyjasniony wyciek po jednym czlowieku.
**Zmiana (decyzja Jeffa: "2 i 3"), gra zamknieta, kopie obok:**
1. WYLECZONE: wszystkie 35 czynnych ognisk partii gracza oznaczone jako `is_recovered = true`, `is_treated = true`, `disease_progress = 0`, `infected_troop_count = 0`. Wpisy zostaja w pliku (mod dostaje przez nie odpornosc sezonowa), tylko sa martwe. Reszta swiata NIETKNIETA - 765 czynnych ognisk u AI zostaje. Kontrola po zmianie: partia gracza 35 wpisow, 0 czynnych, 0 zarazonych; JSON parsuje sie.
2. SMIERTELNOSC: `DiseaseMaxDeathChance` 0.3 -> 0.0 w MCM AIInfluence. Choroby dzialaja dalej (kary do morale, predkosci, skilli), ale nikt od nich nie umiera. `EnableDiseaseSystem` zostaje `True` - Jeff nie chcial wylaczac calego systemu.
**Ryzyko / co sprawdzic:** w logu maja zniknac linie `KSIEGA LUDZI: ... DiseaseEffectSystem.KillTroopsFromDisease`. Jesli kumulacja wroci (znow kilkanascie ognisk tej samej choroby na partii), to blad AIInfluence i trzeba bedzie albo wylaczyc `EnableDiseaseSystem`, albo zalatac ten mod. Cofniecie: `disease_instances.json.bak-2026-09-20-przed-czyszczeniem` i `AIInfluenceSettings.json.bak-2026-09-20-przed-smiertelnoscia`. UWAGA: MCM nadpisuje swoj json przy zamknieciu gry - jesli Jeff ruszy suwaki chorob w grze, wartosc 0.0 moze wrocic do 0.3.
**Status:** WYKONANE 2026-09-20 (gra zamknieta; nasze DLL bez zmian - dziala wgrana wczoraj ManLedger, Armoury.dll md5 9bd31316a62a3b98ecdeccd3307a260b). DO SPRAWDZENIA po wczytaniu.


## 2026-09-19 (6) - KSIEGA LUDZI: kazdy ubytek z partii gracza trafia do logu z NAZWA KODU, ktory go zabral. Koniec zgadywania
**Mod:** Armoury | **Pliki:** `Armoury/src/ManLedger.cs` (NOWY), `Armoury/src/SubModuleMain.cs` (rejestracja)
**Zgloszenie (Jeff 19.09, trzeci raz tego dnia):** "Czemu nadal trace ludzi, jestem w oblezeniu, save rozpoczalem od 318, teraz mam 313".
**Co log mowil przed ta zmiana (sesja `Armoury-2026-09-19_15-58-07.log`, oblezenie The Rills):**
- `[15:59:08] DesertionLaw: partia gracza 318 ludzi, limit 320, morale 100, glod: nie.` - dezercja z morale wykluczona (progi t1<25).
- Prawo zaleglego zoldu po poprawce dziala poprawnie i partii gracza NIE dotyczy: `[16:02:14] WarLedger: CharacterObject_9332_party_1 traci 1 ludzi (zold niewyplacony 4 dni, liczone jak 2; zalegosc 1, dzienny zold 38, kasa klanu 0)` - to lord AI z PUSTA kasa, czyli dokladnie ten, kto placic nie moze. Zadnej linii `PARTIA GRACZA`.
- Zarazy obozowej brak: zero linii `CampFever` w tej sesji (oblezenie dopiero sie zaczelo - `przygotowania obozu 10%, 17%, 23%, 30%`, a zaraza wstaje od 9 dnia oblezenia).
- Zostaly wiec mechanizmy, ktorych NIC u nas nie loguje: vanillowy ostrzal oblezniczy (`SiegeEventCampaignBehavior.KillRandomTroopsOfEnemy` - kazde trafienie machiny obroncy to 1 zabity, zniszczona machina 2) i cokolwiek innego w 45 modach.
**Dlaczego diagnostyka, a nie kolejna hipoteza:** szukanie winnego po objawach zawiodlo w tej sprawie trzy razy - raz zrzucilem strate na bitwe, raz (slusznie, ale tylko w czesci) na nasze prawo zoldu, a i tak zostawaly ubytki bez wyjasnienia. Doszedl do tego blad metody: `Log.Player` nie pisze do pliku, wiec "brak linii w logu" bylo bezwartosciowym dowodem.
**Zmiana:** prefix Harmony na `TroopRoster.AddToCountsAtIndex`. To JEDYNE gardlo - dekompilacja 19.09 pokazuje, ze `AddToCounts`, `RemoveTroop` i `WoundTroop` wszystkie do niego wolaja, wiec jedna latka lapie kazda droge ubytku. Prefix, a nie postfix, bo po zmianie wpis potrafi zniknac (`removeDepleted`) i nie byloby juz komu odczytac nazwy oddzialu. Warunki: TYLKO `MobileParty.MainParty.MemberRoster` i TYLKO `countChange < 0` (rany nie sa ubytkiem) - reszta swiata nie placi za to nic. Linia w logu: `KSIEGA LUDZI: -N <oddzial>[, w bitwie | w obozie oblezniczym | w osadzie X] | zabral: <Typ.Metoda <- Typ.Metoda <- Typ.Metoda> | ta przyczyna dzis: M ludzi w K ruchach | razem w sesji: T | partia ma teraz P.` Nazwa winowajcy pochodzi z trzech pierwszych klatek stosu spoza rostera, Harmony i nas samych. Zeby bitwa nie zasypala pliku, ta sama przyczyna dostaje pelny wpis trzy razy, potem co dwudziesty - ale liczniki biegna caly czas i sa w kazdej linii.
**Ryzyko / co sprawdzic:** przy starcie `ManLedger: ksiega ludzi czynna ...` (czerwona flaga: `AddToCountsAtIndex nieznalezione`). Po kazdej stracie linia `KSIEGA LUDZI:`. Koszt: budowa sladu stosu przy kazdym ubytku partii gracza - w bitwie to moze byc kilkaset wywolan, dlatego slad jest bez plikow i numerow linii, a wpisy sa dlawione. Jesli log spuchnie albo gra zwolni w bitwie, podniesc prog dlawienia albo wylaczyc latke.
**Status:** WGRANE 2026-09-19 (Armoury.dll md5 9bd31316a62a3b98ecdeccd3307a260b, build rc=0, gra zamknieta; poprzednie DLL obok jako `Armoury.dll.bak-2026-09-19-przed-ksiega-ludzi`). DO SPRAWDZENIA po wczytaniu - to ta zmiana ma dac odpowiedz, kto zabiera ludzi w oblezeniu.


## 2026-09-19 (5) - ZASADA GENERALNA, ciag dalszy: dwie realne dziury w liczeniu (chwiejny porzadek podazy i sztywne x2 na amunicje) plus jeden czytelny komunikat zamiast dwoch czerwonych alarmow
**Mod:** Armoury | **Pliki:** `Armoury/src/QuartermasterLaw.cs` (FitFor: ModQuality/ModId + komparator podazy, SlotsOfType + mnoznik amunicji; PurgeUnusable: komunikat dla gracza)
**Zgloszenie (Jeff 19.09):** "jak wkladam np. 1 luk i lucznik go bierze to luk mi znika, a jak lucznik ma luk to nastepuje wymiana nowy luk znika w stash a stary luk lucznika pokazuje sie w stash. To tylko przyklad chodzi o generalna zasade."
**Ustalenie porzadkujace:** ksiega wkladow NIE MA PAMIECI. Przy kazdym otwarciu i zamknieciu `PurgeUnusable` przepisuje ja od zera: `targetOwn = Total - Used`. Cala wlasnosc sprowadza sie wiec do jakosci liczby `Used` - i to tam siedzialy bledy.
**BLAD 1 - KOMPARATOR PODAZY NIE MIAL PORZADKU CALKOWITEGO (setki sztuk przewracalo wlasnosc przy kazdym otwarciu).** `supply.Sort` mial dwa kryteria (`Difficulty`, `RangedRank.Key`) i oba czytaly sam `ItemObject` - slepe na modyfikator. ROT ma cale bloki sztuk o IDENTYCZNYCH statach (casterly_heavy_helm / north_heavy_helmet / grafton_helmet3: t6, wymog 175, wartosc 24364; szesc par rekawic t5/140; rot_horse_saddle2 i rot_horse_saddle4 co do kolumny). Dla nich komparator zwracal 0, wiec o tym, ktore id "nosza ludzie", decydowala KOLEJNOSC WPISOW W ROSTERZE - a ta zmienia sie przy kazdym chowaniu przez escrow (od konca) i oddawaniu depozytu (na koniec). Dowod z logu `Armoury-2026-09-19_14-04-44.log`: `[14:08:08] porzadek HeadArmor: casterly_heavy_helm -9, grafton_helmet3 -3, north_heavy_helmet +11` i `[14:09:20]` DOKLADNA ODWROTNOSC; `Armoury-2026-09-19_07-46-41.log [10:42:24] 240 szt. na liste gracza, 246 na stan wojska [HeadArmor +23/-23, BodyArmor +18/-18, LegArmor +81/-81, HandArmor +79/-79, Cape +33/-33]` - symetryczne +N/-N to podpis tego bledu. Dla gracza wygladalo to jak ciagle zabieranie i oddawanie losowego sprzetu. Sort stabilny by tego NIE naprawil (stabilny zachowuje kolejnosc wejscia, a wejsciem jest wlasnie permutowany roster). **Zmiana:** komparator domkniety do porzadku calkowitego - po `Difficulty` i `RangedRank.Key` decyduje jakosc modyfikatora (lepszy egzemplarz na ludzi, gorszy graczowi - dokladnie zasada Jeffa), a na samym koncu alfabet `StringId` i id modyfikatora, zeby wynik nie zalezal od kolejnosci rostera.
**BLAD 2 - STYWNE x2 NA AMUNICJE ZEROWALO WLASNOSC GRACZA NA STALE.** `FitFor` liczyl kazdemu strzelcowi i oszczepnikowi DWA stosy (mnoznik z 14.09, wprowadzony po jednym screenie "Arrows 86/172"). Lucznicy ROT maja we wzorcu faktycznie dwa sloty kolczanow, ale oszczepnicy maja JEDEN pek. Skutek w logu 19.09, szesc przebiegow z rzedu: `papier Thrown: potrzeba 156 (dopasowanie liczy 156), polka 153, udzwigna 153` - popyt WYZSZY niz polka, wiec `targetOwn = Total - Used = 0` dla kazdego id i ksiega oszczepow gracza byla zerowana na stale. **Zmiana:** `SlotsOfType(c, type)` liczy, ile slotow broni (0-3) wzorzec oddzialu trzyma na ten typ, nigdy mniej niz 1. `WornFor` (raport brakow, `n.Bows * 2` itd.) ZOSTAWIONY bez zmian - osobna zmiana po sprawdzeniu efektu.
**BLAD 3 (R4) - GRACZ NIE MIAL JAK ZOBACZYC WYMIANY.** Szly dwie osobne CZERWONE linie, ktorych nic nie wiazalo, a ta o zabranych sztukach nie podawala ANI JEDNEJ nazwy. Do tego limit trzech nazw byl globalny dla wszystkich 16 typow, a wielokropek leciał przy `names.Count >= 3`, czyli klamal o obcieciu takze przy dokladnie trzech. **Zmiana:** jedno zdanie w normalnym kolorze, obie strony wymiany z nazwami: `QM: the men took N pcs of yours (...); M pcs no man wears are yours in the stash (...).` Gdy nic nie wyparto: `nothing was displaced - they had empty hands`.
**OBALONA HIPOTEZA (nie powtarzac):** "stary luk wraca do magazynu dopiero przy przydziale DTE przed bitwa". Nieprawda - na mapie kampanii CALY sprzet, takze noszony, lezy fizycznie w `ArmyArmory.Armory`; DTE zdejmuje sztuki dopiero przy spawnie agenta (`SpawnAgentPatch` -> `ArmyArmory`) i oddaje po bitwie. Stary luk juz tam lezy, nie ma dokad wracac. Droga powrotna byla zamknieta po stronie KSIEGI, nie fizycznie.
**Ryzyko / co sprawdzic:** linie `porzadek <Typ>` maja przestac sie odwracac miedzy kolejnymi otwarciami, a symetryczne `+N/-N` per typ maja zniknac; `papier Thrown` ma pokazac `potrzeba` nizsze niz `polka`, a nie odwrotnie; po kazdym porzadku jedno zdanie `QM: the men took ...; ... are yours in the stash (...)`. Jesli `porzadek` nadal skacze tam i z powrotem, znaczy, ze zrodlem permutacji jest cos poza escrow.
**ZOSTAJE DO DECYZJI (nie zrobione):** typy spoza `KitTypes` (16 z 26 wartosci enuma - proca, pociski, Goods, sztandary) nie dostaja zadnej korekty, wiec sztuka, ktora trafila na polke NIE z reki gracza, jest dla niego niewidoczna na zawsze; w logach nie potwierdzone, ze takie sztuki realnie wystepuja. Osobno: `GatherFallen` obiecuje w komunikacie wlasnosc, ktorej ksiega potem nie utrzymuje.
**Status:** WGRANE 2026-09-19 (Armoury.dll md5 0dc96e8ab404b73e988b31b32b49bdca, build rc=0, gra zamknieta; poprzednie DLL obok jako `Armoury.dll.bak-2026-09-19-przed-porzadkiem`). DO SPRAWDZENIA po otwarciu zbrojowni.
...
**USTALENIE NR 1 - NIC FIZYCZNIE NIE PRZEPADLO.** Suma z linii `papier <typ>: ... polka N` (liczona PRZED chowaniem) w `Armoury-2026-09-19_14-04-44.log`: 14:08:08 polka 10067, 14:09:10 **3252**, 14:09:30 **10040**, 14:22:33 3171, 14:23:09 **6911**, 14:28:14 6989, 14:29:19 3177, 14:29:51 3159. Przedmioty WRACAJA (+6788 szt. miedzy 14:09:10 a 14:09:30, +3740 miedzy 14:22:33 a 14:23:09) - z niczego nie przybywaja, wiec lezaly w sakwach gracza. Niezalezny licznik `AWANS ... magazyn: N` (liczy CALY roster bez filtrow) zgadza sie co do sztuki: 3252 o 14:08:47 = suma papieru o 14:09:10; 3177 o 14:28:58 = suma o 14:29:19. `SmeltTab: dolozono 390 pozycji (pancerze)` o 14:14:54 liczy `MainParty.ItemRoster` - 390 stosow pancerza bylo wtedy w SAKWACH gracza (rano, gdy leżały na polce, ten sam licznik pokazywal 4). Czesc wojskowa nie drgnela (3167/3167/3171/3177/3159). Zero ERROR, zero `TrimWarStores`, zero `ducha` w tej sesji. Bilans `porzadek w skarbcu` wychodzi NA KORZYSC gracza: 19.09 popoludnie +717/-620 (netto +97), rano +5949/-4855 (+1094), 18.09 +1589/-1084 (+505).
**USTALENIE NR 2 - DLACZEGO LISTA JEST PUSTA.** `na liscie gracza` to nie polka, tylko `min(polka, ksiega)` po schowaniu. Gdy Jeff wynosi swoj sprzet do sakw, `BookPostfix` zdejmuje go z ksiegi (`StockWithdraw`), wiec przy nastepnym otwarciu ekran mowi `(pusto)` obok `skarbiec wojskowy (3159 szt.) schowany` - te 3159 sztuk TAM JEST, tylko jako wlasnosc wojska. Stad wrazenie, ze "stash wciagnal wszystko".
**USTALENIE NR 3 - "NIC W ZAMIAN" JEST PRAWDA OD DAWNA I NIE JEST BLEDEM.** `ProcessSwaps` nie wydaje ANI JEDNEJ sztuki - loguje "QM: no man can use ..." i wola `PurgeUnusable`, ktory tylko przeksiegowuje wlasnosc. Fizyczne wydawanie do sakw umarlo przy przepisaniu barteru 29.08 -> 14.09 ("najpierw braki, potem wymiana", CHANGELOG 14.09). Regresja z 14.09 jest INFORMACYJNA: znikly komunikaty nazywajace to wymiana ("Quartermaster's exchange: ... yours to sell"), zostaly dwie suche linie "QM: N pcs no man wears -> your list" / "QM: N of your pcs -> the men", a przy samym WKLADANIU gracz nie dostaje zadnego komunikatu. UWAGA: wpis w CHANGELOG z 16.09 twierdzacy, ze "wymiana barterowa tez wydaje do sakw", jest NIEPRAWDZIWY - nie opierac na nim diagnoz.
**USTALENIE NR 4 - REALNY BLAD, MOJ, z 16.09: ZAPADKA ROZKAZU.** W `PurgeUnusable` rozkaz z ksiegi musztry byl sprawdzany na POJEDYNCZYM wpisie rostera, PRZED zsumowaniem korekt per id: dodatnie wpadaly w `continue` i nie dochodzily do sumy, ujemne przechodzily. Ksiega przypietego id mogla wiec tylko MALEC. Dowod: `[14:18:01]` i `[14:18:02]` dwa razy `porzadek w skarbcu - 0 szt. na liste gracza, 2 szt. na stan wojska [Bow +0/-2]`, podczas gdy prawdziwa suma per id wynosila **+5**; licznik `pinnedKept` szedl 123 -> 125 -> 127. Skutek: cztery `weirwood_bow` zeszly graczowi z ksiegi bez rekompensaty. Przypiete id to `weirwood_bow`, `ravens_teeth_longbow`, `ravens_teeth_arrows`. To NIE tlumaczy wielkich znikniec (o 14:28:14 i 14:29:19 linia rozkazow mowi "noszone z rozkazem na stan wojska: (brak)", a rozpiska 110 szt. nie zawiera zadnego przypietego id) - ale jest bledem i zabieral wlasnosc.
**Zmiana:** korekty sumowane sa NAJPIERW per id, a rozkaz z ksiegi stosowany dopiero do WYNIKU: netto na plus zostaje na stanie wojska (czeka na ludzi z rozkazem), netto na minus schodzi z ksiegi gracza jak kazde noszone sztuki. Koniec jednokierunkowej zapadki.
**Ryzyko / co sprawdzic:** po otwarciu zbrojowni linia `rozkazy - noszone z rozkazem na stan wojska: ...; nienoszone z rozkazem zostaja na stanie: N szt.` ma dalej wystepowac, ale `porzadek Bow` nie moze juz pokazywac samych minusow przy dodatniej sumie per id. Jesli licznik `nienoszone ... zostaja na stanie` bedzie rosl bez konca, znaczy, ze rozkazow jest wiecej niz sprzetu i trzeba je przejrzec w ksiedze musztry.
**DO DECYZJI JEFFA (nie zmieniane):** czy kwatermistrz ma WYDAWAC wyparty sprzet fizycznie do sakw (ryzyko przeciazenia), czy zostawiac na liscie w stashu (jak dzis) i tylko jawnie mowic, ile sztuk czeka na odbior. Dzis nie mowi nic przy wkladaniu - to glowny powod wrazenia "nic nie dostalem".
**Status:** WGRANE 2026-09-19 (Armoury.dll md5 e42ad3822d0f8e1b9aa445b6b670860e, build rc=0, gra zamknieta; poprzednie DLL obok jako `Armoury.dll.bak-2026-09-19-przed-zapadka`). DO SPRAWDZENIA po otwarciu zbrojowni.


## 2026-09-19 (2) - ZNALEZIONE: to NASZ WarLedger zjadal Jeffowi elite. "Zalegly zold" pytal o POLE, nie o pieniadze, licznik dni rosl bez konca, a strata gracza NIE TRAFIALA DO LOGU
**Mod:** Armoury | **Pliki:** `Armoury/src/WarLedger.cs` (OnDaily), `Armoury/src/Settings.cs` (+McmSettings: WagesDesertMaxDays)
**Zgloszenie (Jeff 19.09):** najpierw "czemu mi ubywa zolnierzy, mialem 320 a mam 319", potem "KURWA czemu znowu ubylo mi 6 ludzi, mialem 320 i nagle mam 314".
**Dlaczego nie znalazlem tego za pierwszym razem (to jest osobna wina):** `Log.Player` (Armoury/src/Log.cs:69-77) wywoluje WYLACZNIE `InformationManager.DisplayMessage` - pokazuje komunikat w grze i NIC nie zapisuje do pliku. W `WarLedger.OnDaily` strata partii GRACZA szla przez `Log.Player`, a tylko partie AI przez `Log.Info`. Przeszukalem wiec logi po "Unpaid and unbound", nie znalazlem ani jednego trafienia i uznalem WarLedger za oczyszczonego. Log mowil prawde o AI i milczal o graczu.
**Dowod z logu (`Modules/Armoury/Armoury-2026-09-19_14-04-44.log`):** `[14:34:41] WarLedger: CharacterObject_9332_party_1 traci 1 ludzi (zold niewyplacony 19 dni)`, `[14:35:10] ROTlord_323_party_1 ... 20 dni`, `[14:36:17] ROTlord_342_party_1 ... 21 dni`, `[14:37:18] ... 22 dni`, `[14:37:47] ... 23 dni` - ten sam lord traci czlowieka KAZDEGO dnia, a licznik zwloki rosnie monotonicznie i nigdy nie wraca do zera. Rownolegle papier kwatermistrza pokazuje kurczenie sie partii gracza: `Thrown ... potrzeba 158` (14:28:14) -> `156` (14:34:41) -> `154` (14:37:47).
**Przyczyna (trzy bledy naraz, wszystkie nasze):**
1. `MobileParty.HasUnpaidWages` to NIE flaga "dzis nie zaplacono", tylko KWOTA zalegosci - `public float`, `[SaveableField(1006)]`, zapisywana w save (dekompilacja MobileParty 19.09). Raz niespłacona zostaje niezerowa, a przy Banner Kings, ktory przejmuje finanse klanu, potrafi wisiec tak w nieskonczonosc. Nasz warunek `HasUnpaidWages > 0f` czytal wiec "dlug historyczny" jako "dzis nie zaplacono" i naliczal kolejny dzien zwloki KAZDEGO dnia - takze graczowi, ktory ma w skarbcu ponad 500 tys. zlota.
2. Brak sufitu eskalacji: `pct = WagesDesertPercentPerDay (0.5%) * over`, gdzie `over` rosnie bez granic. Przy 23 dniach zwloki to 10.5% skladu dziennie dla AI (polowa stawki) i 11.5% dla gracza. Przy 320 ludziach dawaloby ponad 30 ludzi na dzien. Jeff stracil 1, potem 6 - to dokladnie ten rozped (6/320 = 1.9%, czyli ok. czwarty dzien ponad laske).
3. Dezercja bierze ELITY PIERWSZE (`DesertElitesFirst`, najwyzszy tier naprzod) - zgodnie z zamowieniem z 31.08, ale wlasnie dlatego znikali najlepsi ludzie i Jeff to zauwazyl.
**Zmiana:**
1. Dluznikiem jest tylko ten, kto NIE MA CZYM ZAPLACIC: przed naliczeniem dnia zwloki sprawdzamy `payer.Gold >= max(1, mp.TotalWage)` (klan partii: `ActualClan`, a w zapasie klan dowodcy). Kto ma w kasie na dzienny zold tej partii - dlugu nie ma, licznik zeruje sie, nikt nie odchodzi. To zdejmuje karę z gracza i z kazdego wyplacalnego lorda AI.
2. Sufit eskalacji `WagesDesertMaxDays` (domyslnie 8): `over` nie rosnie powyzej tej wartosci, wiec maksymalna stawka to 4% skladu dziennie (AI 2%), a nie kilkanascie procent.
3. KAZDY ubytek trafia teraz do PLIKU, takze u gracza: `WarLedger: PARTIA GRACZA traci N ludzi (zold niewyplacony D dni, liczone jak O; zalegosc Z, dzienny zold W, kasa klanu G)`. Komunikat w grze zostaje bez zmian.
**Ryzyko / co sprawdzic:** po wczytaniu w logu NIE powinno byc linii `WarLedger: PARTIA GRACZA traci ...` dopoki masz zloto; linie o partiach AI maja przestac dotyczyc lordow wyplacalnych, a licznik dni ma przestac rosnac w nieskonczonosc (zostanie tylko u naprawde bankrutujacych). Jesli linia o graczu MIMO TO sie pojawi, poda dokladnie: ile dni, jaka zalegosc, jaki dzienny zold i ile klan ma w kasie - z tego od razu widac, czy to nasz blad, czy realne bankructwo. Suwak `Wages Desert Max Days` w MCM Armoury. Co NIE zostalo zrobione: nie ruszamy `DesertElitesFirst` (elity dalej pierwsze - to zamowienie Jeffa z 31.08) i nie oddajemy utraconych ludzi; strata z poprzednich dni zostaje w save.
**Do sprawdzenia osobno:** dlaczego `HasUnpaidWages` w ogole jest niezerowe u gracza z pelnym skarbcem - podejrzany jest model finansow Banner Kings, ktory zastepuje vanillowy `DefaultClanFinanceModel`. Nasza zmiana uodparnia nas na to, ale zrodlo zostaje.
**Status:** WGRANE 2026-09-19 (Armoury.dll md5 e1091085d9a86a4f1b0327210cf92ba6, build rc=0, gra zamknieta; poprzednie DLL obok jako `Armoury.dll.bak-2026-09-19-przed-zoldem`). DO SPRAWDZENIA po wczytaniu.


## 2026-09-19 - CRASH na tiku klanu: KOBIETA na tronie kladzie gre, bo ZADNA z 24 kultur ROT nie ma zenskiego stroju wladcy; brak szablonu nie zabija juz gry + audyt szablonow w logu
**Mod:** CrashScribe | **Pliki:** `CrashScribe/src/Mends.cs` (DressedOrNot + RobesMissing + RulerRobesAudit + RobesHoles + RobesProbe, blok Install(robes), wpiecie audytu w OnSessionLaunched)
**Zgloszenie (Jeff 19.09):** "ok wyskoczyl crash gry sprawdz".
**Dowod z logu (`CrashScribe/session-2026-09-19_07-46-39.log:3989`, 12:14:26):**
`System.NullReferenceException at Helpers.EquipmentHelper.AssignHeroEquipmentFromEquipment(Hero, Equipment) -> NPCEquipmentsCampaignBehavior.OnRulingClanChanged(Kingdom, Clan) -> CampaignEvents.OnRulingClanChanged -> KingSelectionKingdomDecision.ApplyChosenOutcome_Patch1 -> BannerKings.Managers.Kingdoms.Succession.BKKingElectionDecision.ApplyChosenOutcome -> KingdomElection.ApplyChosenOutcome/ReadyToAiChoose -> KingdomDecisionProposalBehavior.UpdateKingdomDecisions -> DailyTickClan_Patch2 -> CampaignPeriodicEventManager.PeriodicDailyTick -> Campaign.Tick -> MapState.OnTick`. Stan: Winter 15 1090 (dzien 1105), gracz w Barrowton [town_B4], menu `town_wait_menus`, turniej, partia 320 ludzi. Po wyjatku GAME HANG (61 s) i `Dump generation canceled`.
**Przyczyna (dekompilacja + przeliczenie danych 19.09):**
1. `Helpers.EquipmentHelper.AssignHeroEquipmentFromEquipment` nie ma ANI JEDNEGO straznika: pierwsza instrukcja to `equipment.IsStealth`, zaraz potem `hero.StealthEquipment/BattleEquipment/CivilianEquipment`. Null w ktorymkolwiek argumencie = NullReference.
2. `NPCEquipmentsCampaignBehavior.OnRulingClanChanged` (cala metoda, 17 linii) robi cztery takie ubierania i NIE sprawdza, co zwrocil model.
3. `DefaultEquipmentSelectionModel.GetEquipmentsForChangingRuler` -> `GetSuitableEquipmentSet` zbiera roster-y, gdzie `EquipmentCulture == hero.Culture && EquipmentCategories == customFlags` (DOKLADNA rownosc flag; dla kobiety doklada `IsFemaleTemplate`), i konczy na `mBList.GetRandomElement()`, ktore na pustej liscie zwraca null.
4. DZIURA W DANYCH - policzone niezaleznie przez oba przebiegi (skrypt po calym `Modules/**/ModuleData/*.xml`, 22730 elementow `<EquipmentRoster>`, flagi jako ATRYBUTY elementu `<Flags .../>`): zenski strój wladcy (`IsKingdomRulerTemplate` + `IsFemaleTemplate`) maja WYLACZNIE aserai, battania, empire, khuzait, nord, sturgia, vlandia. NIE MA GO ZADNA z 24 kultur ROT: crownlands, dragonstone, freefolk, ghiscari, ibbenese, lyseni, myrish, nightswatch, norvos, pentoshi, qartheen, qohorik, reach, river, sarnor, skagosi, stormlands, summer, tyroshi, vale, valyrian, volantine, whitewalker, yiti - kazda ma tylko wariant meski (`*_bat_template_ruler` / `*_civ_template_ruler`). Zaden plik `.xslt` w Modules tego nie uzupelnia. Strona STAREGO wladcy jest czysta (`IsLordTemplate` ma komplet obu plci).
WNIOSEK: kazda KOBIETA obejmujaca tron w ktorymkolwiek westeroskim albo essoskim krolestwie kladzie gre. Kto konkretnie zostal wybrany - NIE POTWIERDZONE, zaden log nie podaje imienia ani krolestwa; latka to uzupelni (linia z nazwa bohatera, kultura i plcia).
**Czy save byl zablokowany:** NIE. Ostatni zapis (`IronmanPEh4X2ByGArW.sav`, 12:07:41) jest sprzed elekcji - decyzja, w ktorej gracz nie uczestniczy, rozstrzyga sie na pierwszym tiku klanu po jej powstaniu, a ta powstala dopiero po zapisie. ALE: stary monarcha dalej jest stary i chory (BirthAndDeath dziala - `rgl_log` 11:39:42 "Merrett the Gutting-Knife died from The Dock Fever"), wiec elekcja powtorzy sie w ciagu kilku dni gry, a tik klanu chodzi w KAZDEJ klatce mapy, nie o polnocy. Bez latki to byla ruletka.
**Zmiana:** prefix `DressedOrNot` na `Helpers.EquipmentHelper.AssignHeroEquipmentFromEquipment(Hero, Equipment)`: gdy ktorykolwiek argument jest null, ubieranie jest POMINIETE (bohater zostaje w swoim sprzecie), oryginal nie biegnie. Latamy sam helper, bo to jedyne gardlo (16 wywolan w TaleWorlds.CampaignSystem: m.in. HeroCreator, AgingCampaignBehavior, InitialChildGeneration, Hero.AfterLoad, NPCEquipments x4, plus BannerKings, BKROTPatch i StoryMode) i siedzi PONIZEJ modelu - zadziala nawet, gdyby ktos podmienil EquipmentSelectionModel (dzis nikt tego nie robi ani nie patchuje, sprawdzone skanem po wszystkich DLL). Elekcja konczy sie normalnie, krol rzadzi, tylko bez nowej szaty. Do tego `RulerRobesAudit` przy starcie sesji: dla kazdej kultury majacej dzis krolestwo albo zywego bohatera sprawdza osiem kombinacji (wladca/lord/dziecko/nastolatek x obie plcie; wladca i lord pytani o zestaw bitewny i cywilny, dziecko i nastolatek tylko o cywilny) i wypisuje wszystkie dziury - to lista do uzupelnienia danych.
**Ryzyko / co sprawdzic:** przy starcie w logu `Mends: ubieranie bohatera bez zestawu strojow nie kladzie juz gry ...` (czerwona flaga: `NIEZNALEZIONE`) oraz `AUDYT STROJOW: N z M kultur ma dziury w szablonach ... -> river: WLADCZYNI(bitwa i cywil), ...`. Przy elekcji kobiety: `Mends: brak pasujacego zestawu strojow (model oddal null) dla <imie> [kultura X, kobieta] - ubieranie POMINIETE`. Ma NIE byc bloku `PIERWOTNY WYJATEK CRASHA` z ramka `AssignHeroEquipmentFromEquipment`. Skutek uboczny: nowa krolowa nie dostanie korony ani szaty wladcy - zostanie w swoim stroju. To kosmetyka, ale wlasciwym domknieciem jest UZUPELNIENIE DANYCH (dopisac 24 kulturom roster zenski z flagami `IsFemaleTemplate` + `IsKingdomRulerTemplate`, bitewny i cywilny) - osobne zadanie, po potwierdzeniu, ze crash zniknal. Audyt pokaze przy okazji, czy brakuje tez szablonow dziecka i nastolatka (kultura whitewalker) oraz czy kultury bandyckie i minor-factions nie maja zadnych roster-ow.
**Status:** WGRANE 2026-09-19 (CrashScribe.dll md5 f25bbdfa3d56579678480eb8c4990c9e, build rc=0, gra zamknieta; poprzednie DLL obok jako `CrashScribe.dll.bak-2026-09-19-przed-strojami`). DO SPRAWDZENIA po wczytaniu save'a.


## 2026-09-18 (2) - Ta sama osada, druga mina: po zdjeciu pierwszej crash wrocil w UpdateProperties - kafelek partii osady znika teraz z list stron, a podpowiedzi dostaja finalizery
**Mod:** CrashScribe | **Pliki:** `CrashScribe/src/Mends.cs` (SafeSiegeOverlayLists + ScanOverlayTiles + SafeSiegeOverlayProps + FixNaNMorale + SafeSiegeOverlayHint + SafeMenuPartyTooltip, wpiecie w bloku Install(siegeOverlay))
**Zgloszenie (Jeff 18.09):** "odpalilem gre, wszedlem w oblezenie, sprawdz log".
**Dowod z logu (`CrashScribe/session-2026-09-18_16-05-15.log`):**
- `:393` `[16:05:37] Mends: overlay oblezenia zabezpieczony - ...` - latka nr 1 zainstalowana.
- `:1999` `[16:18:20] Mends: overlay menu - partia osady Moat Cailin nie ma MobileParty (x1); w jej rosterze 18 ZDROWYCH Z 18 - link encyklopedii osady podstawiony, NullReference ominiety.` - latka nr 1 ZADZIALALA i domknela wczorajsze "nie potwierdzone": osada to Moat Cailin, a liczba 18 to dokladnie porcja `StrategicCampaignAI145.TryReinforceFragileGarrison` (`RemoveNumberOfNonHeroTroopsRandomly(18)`). HIPOTEZA O TYM MODZIE POTWIERDZONA.
- `:2002-2013` i natychmiast NASTEPNY crash, ten sam konstruktor: `System.NullReferenceException at EncounterMenuOverlayVM.UpdateProperties() -> ..ctor() -> NavalGameMenuOverlayProvider.GetOverlay -> ... -> MapScreen.EnterMenuContext`. Stan: `Menu: menu_siege_strategies`, `Battle: (none)`, `Army: (none)` (czyli MapEvent == null, IsSiege == true).
**Przyczyna:** winna linia `EncounterMenuOverlayVM.cs:734` `num += defenderParty.Party.MobileParty.Morale;` w petli po `DefenderPartyList` (:732-739). Kafelek partii osady, ktory latka nr 1 dopuscila do powstania, stoi na tej liscie i znowu nie ma `MobileParty`. Przeglad calej klasy: takich niestrzezonych dereferencji jest KILKANASCIE - `EncounterMenuOverlayVM.cs:734, 737` (morale i jedzenie obroncow), `:777, 780` (atakujacy), `:1002-1005` (podpowiedz jedzenia), `:1130-1137` (statki), `:1186-1187` (morale), plus `GameMenuPartyItemVM.cs:614` (`Character.HeroObject` przy najechaniu mysza, bo kafelek nie ma ani MobileParty, ani pola Settlement). Latanie ich po kolei to gonienie wlasnego ogona - stad zmiana strategii: usunac kafelek z listy.
**Weryfikacja przed napisaniem kodu (dekompilacja):** `GameMenuPartyItemVM.RefreshProperties` jest BEZPIECZNE dla partii niemobilnej (`:627 if (Party.IsMobile)` oslania `:629-652`, `:784` ma wlasny test) - jedyna dziura to `:620 GetEncyclopediaPageLink`, juz zalatana; `UpdateLists` (:800) i `UpdateProperties` (:718) sa prywatne, bezparametrowe, wolane po kolei z konstruktora (:613-614) i z odswiezenia; `DefenderPartyList`/`AttackerPartyList` to WLASCIWOSCI typu `MBBindingList<T>`, ktory dziedziczy `Collection<T>` i implementuje NIEgeneryczny `IList` (mozna go obslugiwac bez referencji do ViewModelCollection); cztery podpowiedzi to `private List<TooltipProperty> GetEncounterSide*Tooltip(BattleSideEnum)` odpalane LENIWIE z lambd `BasicTooltipViewModel` (:603-610); `ExecuteOpenTooltip` (:602) pokazuje dla osady `ShowTooltip(typeof(Settlement), Settlement)` - nasza galaz robi to samo.
**Zmiana (wszystko w bloku `Mends.Install(siegeOverlay)`, PRZED `return` bloku RealisticBannerlord):**
1. POSTFIX na `UpdateLists` (`SafeSiegeOverlayLists` + `ScanOverlayTiles`): po przebudowie zdejmuje z OBU list kafelki, ktorych `Party != null && Party.MobileParty == null`; idzie od konca, domyka zdjete przez `OnFinalize` (inaczej sluchacze kampanii mnozyliby sie z kazdym odswiezeniem), kafelka wiodacego (`Party == null`) NIE rusza. Przy trwajacej BITWIE (`MainParty.MapEvent != null`) list NIE czysci - tam klatkowy tick porownuje dlugosc list z liczba uczestnikow i przebudowywalby je co klatke; crashu tam nie ma, bo `UpdateProperties` wychodzi od razu na `!IsSiege`.
2. FINALIZER na `UpdateProperties` (`SafeSiegeOverlayProps` + `FixNaNMorale`): gdy lista strony zejdzie do zera, vanilla dzieli zero przez zero i wpisuje do okienka "NaN" - podmieniamy na "-"; kazda inna wywrotka polknieta, pierwsza z pelnym sladem do logu.
3. FINALIZERY na czterech podpowiedziach stron (`SafeSiegeOverlayHint`): wywrotka daje pusta liste wlasciwosci (typ z `__originalMethod.ReturnType` przez `Activator`), nie crash. To jedyna oslona w scenariuszu z trwajaca bitwa.
4. PREFIX na `ExecuteOpenTooltip` (`SafeMenuPartyTooltip`): najechanie mysza na taki kafelek pokazuje podpowiedz OSADY zamiast luskac `Character.HeroObject`.
Latka nr 1 (`SafeMenuPartyLink`) ZOSTAJE - bez niej `UpdateLists` w ogole nie wraca (konstruktor kafelka wola `RefreshProperties` -> `GetEncyclopediaPageLink` JESZCZE wewnatrz `UpdateLists`), wiec postfix nigdy by nie wystartowal.
**Ryzyko / co sprawdzic:** przy starcie musi byc PIEC linii: `overlay oblezenia zabezpieczony`, `listy stron czyszczone`, `UpdateProperties pod finalizerem`, `podpowiedzi stron pod finalizerem (4/4)`, `hover na kafelku ... podpowiedz osady`. Czerwone flagi: `EncounterMenuOverlayVM NIEZNALEZIONY` / `bez UpdateLists` / `bez list stron`, albo `(3/4)` i mniej. Po wejsciu w oblezenie Moat Cailin maja byc DWIE linie z tym samym czasem: ta z latki nr 1 i nowa `kafelek partii bez MobileParty (Moat Cailin, w rosterze 18 zdrowych z 18) zdjety z listy obroncow (x1)`. Brak drugiej = postfix nie wystartowal. Ma NIE byc bloku `PIERWOTNY WYJATEK CRASHA` z ramka `UpdateProperties`. Sygnaly ostrzegawcze: `UpdateProperties - wyjatek polkniety` (filtr czegos nie zlapal, w raporcie bedzie slad), `podpowiedz oddana pusta` (mina w podpowiedziach byla realna), `overlay bitwy - ... trwa bitwa: list NIE czyscimy` (inny scenariusz do przemyslenia), licznik `x` rosnacy o setki w sekunde (przebudowa co klatke). W grze: `-` przy morale obroncow znaczy, ze po zdjeciu kafelka lista zostala PUSTA, czyli te 18 osob to cala zaloga Moat Cailin; licznik obroncow obok paska sily nadal ich liczy (vanilla liczy go przed nami) i realnie beda sie bic w szturmie.
**Czego NIE naprawiamy tu (osobne zadanie po potwierdzeniu, ze menu przestalo padac):** przyczyna, czyli `StrategicCampaignAI145.TryReinforceFragileGarrison`, ktory dalej sypie ludzi do `settlement.Party.MemberRoster` na kazdej fortyfikacji (jego bramka czyta `EstimatedStrength` partii niemobilnej, ktore z definicji wynosi 0, wiec nigdy nie hamuje). Znana, nieoslonieta mina: `GameMenuOverlay.cs:274` (`_contextMenuItem.Party?.MobileParty.Army` - `?.` na Party, gole na MobileParty) - odpala sie przy KLIKNIECIU kafelka; po naszym usunieciu kafelka nie da sie go kliknac, ale w scenariuszu z trwajaca bitwa (gdzie list nie czyscimy) klikniecie wywali gre i zaden finalizer tego nie zlapie. Nie przeskanowano ~25 z 45 modow pod katem innych zapisow do rostera osady; `AIInfluence.dll` jest zaciemniony i nieczytelny.
**Status:** WGRANE 2026-09-18 (CrashScribe.dll md5 29d34b15b4e93e53c501026f5697436e, build rc=0, gra zamknieta; poprzednie DLL obok jako `CrashScribe.dll.bak-2026-09-18-przed-overlayem-2`). DO SPRAWDZENIA po wejsciu w oblezenie Moat Cailin.


## 2026-09-18 - CRASH przy "join / continue the siege": kafelek partii OSADY w overlayu menu oblezenia (wada w kodzie gry), zabezpieczony prefiksem w CrashScribe
**Mod:** CrashScribe | **Pliki:** `CrashScribe/src/Mends.cs` (SafeMenuPartyLink + wpiecie w Install)
**Zgloszenie (Jeff 18.09):** "jak kliknal join to continue the siege, gra zrobila crasha".
**Dowod z logu (`CrashScribe/session-2026-09-18_13-20-42.log`, 15:21:52, "PIERWOTNY WYJATEK CRASHA"):**
`System.NullReferenceException at GameMenuPartyItemVM.GetEncyclopediaPageLink() -> RefreshProperties() -> ..ctor(Action, PartyBase item, bool) -> EncounterMenuOverlayVM.UpdateLists() -> ..ctor() -> NavalDLC.View.Overlay.NavalGameMenuOverlayProvider.GetOverlay -> GameMenuOverlayFactory.GetOverlay -> GauntletMenuOverlayBaseView.OnInitialize -> MenuViewContext.CheckAndInitializeOverlay -> MapScreen.EnterMenuContext/UpdateMenuView/OnFrameTick`; stan gry: `Menu: menu_siege_strategies`, partia 300 ludzi, w polu. Silnik (`ProgramData/.../logs/rgl_log_3528.txt:49108-49109`): 15:21:50.849 menu `join_siege_event`, 15:21:52.220 `menu_siege_strategies`. Potem GAME HANG (61 s) i `Dump generation canceled`.
**Przyczyna (dekompilacja 18.09, sledztwo 6-watkowe + weryfikacja adwersarialna):**
1. `GameMenuPartyItemVM.GetEncyclopediaPageLink` ma ZEPSUTE strazniki: `if (party == null || !party.MobileParty.IsCaravan)` - sprawdza partie, a luska `MobileParty` (i tak samo dla `IsGarrison`, `IsMilitia`, `IsVillager`). Dla partii OSADY `MobileParty` jest null (`PartyBase.IsMobile => MobileParty != null`) - NullReference. To wada TaleWorlds, ZADEN mod jej nie lata ani nie nadpisuje (skan IL wszystkich DLL w `Modules`: `GameMenuPartyItemVM` referuje tylko `SandBox.GauntletUI.AutoGenerated.*`).
2. Partia osady trafia na liste obroncow z vanilli: `EncounterMenuOverlayVM.UpdateLists` -> `SiegeEvent.GetInvolvedPartiesForEventType` -> `Settlement.GetInvolvedPartiesForEventType` -> `DefaultEncounterModel.GetDefenderPartiesOfSettlement` -> `Town.GetDefenderParties`, ktore robi `yield return base.Settlement.Party;` JAKO PIERWSZA pozycje (`Town.cs:421`). Offset z logu `UpdateLists() IL+0x05F8` wypada w petli OBRONCOW (tuz przed `newobj GameMenuPartyItemVM`, po `get_DefenderPartyList`), nie atakujacych.
3. W czystej grze pozycja odpada na filtrach rostera (`MemberRoster.Count > 0`, a przy oblezeniu takze `TotalHealthyCount > 0`), bo roster partii miasta/zamku jest pusty - garnizon to osobna `MobileParty`. Crash wymaga, zeby ktos wsadzil do rostera OSADY zywych ludzi.
4. KTO: jedyny taki kod znaleziony w calej instalacji to `StrategicCampaignAI145Behavior.TryReinforceFragileGarrison` - `settlement.Party.MemberRoster.Add(...)` zamiast `Town.GarrisonParty`, porcja do 18 zdrowych, cooldown 3 dni na osade, tick co 4 h, bez sprawdzania `SiegeEvent`. Jego bramka "za duzy garnizon" czyta `settlement.Party.EstimatedStrength`, a `PartyBase.CalculateEstimatedCurrentStrength` zwraca 0 dla partii niemobilnej - wiec nigdy nie hamuje. NIE POTWIERDZONE RUNTIME (mod nic nie loguje, nazwa osady nie pada w zadnym logu; z pozycji gracza 383.49/696.89 wychodzi Moat Cailin, `castle_B1`) - to domknie dopiero linia z logu naszej latki.
5. NavalDLC w stosie jest NIEWINNY: `NavalGameMenuOverlayProvider.GetOverlay` tworzy vanillowy `EncounterMenuOverlayVM` (ten sam IL co `DefaultGameMenuOverlayProvider`, offset `IL+0x0004` = ten `newobj`); w stosie jest tylko dlatego, ze fabryka iteruje providery od konca. Nasze 5 modow: zaden nie pisze do `MemberRoster` partii osady (16 punktow mutacji rosterow sprawdzonych).
**Zmiana:** prefix na `GameMenuPartyItemVM.GetEncyclopediaPageLink` (`SafeMenuPartyLink`): gdy `Party != null` i `Party.MobileParty == null`, podstawiamy link do encyklopedii OSADY i NIE wpuszczamy oryginalu (wynik metody idzie do `EncyclopediaCursorEffect` i `ExecuteOpenEncyclopedia`, wiec prawy klik prowadzi na strone osady). Oryginalu nie wolno wpuscic nawet z poprawionymi straznikami, bo kilkanascie linii dalej czeka nieoslonione `Party.Owner` -> `Settlement.OwnerClan.Leader`. Dla partii mobilnych zero zmian (prefix zwraca true). Typ brany przez `AccessTools.TypeByName` (CrashScribe nie referencjonuje `...ViewModelCollection`), pole `Party` to publiczne POLE (nie wlasciwosc), FieldInfo rozwiazywany raz w `Install`. Blok wpiety PRZED blokiem RealisticBannerlord, bo tamten ma `return;` przy braku moda i wszystko za nim by sie nie zainstalowalo.
**Ryzyko / co sprawdzic:** przy starcie w logu `Mends: overlay oblezenia zabezpieczony - ...`; jesli zamiast tego `GameMenuPartyItemVM NIEZNALEZIONY` - dodac `<Reference>` do `CrashScribe.csproj` i patchowac typ wprost. Po wejsciu w oblezenie: `Mends: overlay menu - partia osady <NAZWA> nie ma MobileParty (x1); w jej rosterze N zdrowych z M` - TO JEST BRAKUJACY DOWOD: poda nazwe osady i liczebnosc (N ~18 albo wielokrotnosc 18 = hipoteza StrategicCampaignAI145 potwierdzona). Potem `grep GetEncyclopediaPageLink` po nowych session-*.log ma dawac ZERO. Latka NIE czysci danych - ci ludzie dalej siedza w rosterze osady, beda widoczni w overlayu jako osobna partia obroncow (oznaczona jako neutralna, bo `RefreshRelationStatus` liczy frakcje przez `MobileParty` - vanillowy skutek uboczny) i realnie zgina w szturmie; roster kasuje sie przy zdobyciu osady (`KingdomManager.SiegeCompleted`). Uwaga osobna (NIE ruszane teraz): blok `rbSiege` z 17.09 siedzi ZA tym samym `return;`, wiec bez RealisticBannerlord by sie nie zainstalowal - u Jeffa dziala, bo mod jest zaladowany.
**Status:** WGRANE 2026-09-18 (CrashScribe.dll md5 d8a35a6c7220912ccd8187ec2d5e46e1, build rc=0, gra zamknieta; poprzednie DLL obok jako `CrashScribe.dll.bak-2026-09-18-przed-overlayem`). DO SPRAWDZENIA po wejsciu w to samo oblezenie.


## 2026-09-17 - "Model Innego jest strasznie slaby - moge zrobic go jak czlowieka z odpowiednimi oczami, skora i wlosami?": rasa whitewalker dostaje ludzkie siatki i palete Innego (skins.xml w ROT-Content)
**Mod:** patch cudzego modu (ROT-Content) | **Pliki:** `patches/ROT-Content/skins_whitewalker_human.py` (NOWY, skrypt deterministyczny; wynik wgrany do `Modules\ROT-Content\ModuleData\skins.xml`, oryginal obok jako `skins.xml.bak-2026-09-17-przed-ludzkim-innym`)
**Zgloszenie (Jeff 17.09):** "jak tworze Innego, to moge zrobic go jak czlowieka, tylko wybrac mu odpowiednie oczy, skore i wlosy, bo ten model Innego jest strasznie slaby"; potem "tak, zrob; kobiety nie".
**Jak ROT tworzy Innego (dekompilacja):** `OthersPatches.patch11` (KillCharacterAction.ApplyByExecution, gracz w klanie Innych): `victim.CharacterObject.Race = mezczyzna ? 1 : 2`, kultura WhiteWalkers, dolacza do partii; to samo przy braniu jencow przez klan Innych (`patch7`, Race = 1) i w `ROTOthersCampaignBehavior` (notable/lordowie). Rasa 1 = "whitewalker", rasa 2 = "wight" (kobiety zostaja upiorami). `ROTRaceMap` zapisuje rasy bohaterow w save i przywraca przy wczytaniu. Rasa decyduje o siatkach (skins.xml): meskie skiny "whitewalker" adult/teenager mialy wlasne siatki `whitewalker_body/head/hands/feet` i tekstury twarzy `whitewalker3`; zenskie skiny tej rasy juz od autora ROT uzywaja ludzkich siatek (`body_female_a`, `head_female_a`). Reszta blokow (deform_keys 192, hair 173, beard 207, tattoo 279, mouth) jest IDENTYCZNA z ludzkim skinem - ROT skopiowal ludzki i podmienil siatki. skins.xml czyta silnik FaceGen z kazdego modulu wprost, nie MBObjectManager - xslt nie dziala, stad edycja pliku.
**Zmiana (rasa whitewalker, rasa wight NIETKNIETA):** (1) meskie skiny adult i teenager: siatki = ludzkie meskie z Native (`body_male_a/head_male_a/hands_male_a/feet_male_a`; teen `body_male_a_teen` + ludzkie rece/stopy), bloki `<face_textures>` i `<eyebrow_meshes>` = ludzkie (tekstury twarzy sa robione pod siatke glowy; ww mial 10 brwi zamiast 20, teen 0), glos `male_whitewalker` i `min_scale 1.07` zostaja; (2) WSZYSTKIE 10 skinow rasy: kazdy blok palety (takze dwupunktowe, ciemne, zagniezdzone w `<face_textures>`/`<face_texture>` dla poszczegolnych tekstur twarzy) = paleta Innego: skora (1.00,1.00,1.00)->(0.80,0.86,0.92), wlosy (1.00,1.00,1.00)->(0.72,0.76,0.82), oczy (0.62,0.90,1.00)->(0.18,0.48,0.90). Klucz twarzy nawroconego lorda trzyma jego rysy i przesuniecia kolorow 0..1 - te trafiaja teraz w blada palete, wiec kazdy meski Inny to blady, bialowlosy czlowiek z lodowymi oczami bez recznej edycji. Proba: 42 zmiany, 956 punktow palet w rasie, 0 poza gama; XML parsuje sie; rasy w pliku bez zmian (whitewalker, wight, giant - indeksy 1/2/3, na ktorych polega ROT). Kobiety (rasa 2 = upior) bez zmian - decyzja Jeffa.
**Ryzyko / co sprawdzic:** Nocny Krol i lordowie Innych (`ROT_lords.xml`, race=whitewalker) tez dostana ludzki wyglad - to ta sama rasa. Helmy/zbroje na ludzkiej siatce siedza normalnie. Sprawdzic w grze: nawrocony lord albo Nocny Krol w encyklopedii/bitwie - blady czlowiek, biale wlosy, niebieskie oczy; jesli twarz ma artefakty (tekstura), cofnac plik z .bak. Dzieci Innych (BirthAndDeath) - skiny child/tween/toddler tej rasy mialy juz ludzkie siatki, dostaly tylko palete.
**CRASH 15:43 i poprawka (Jeff: "ok crash gry"):** `rgl_log_17644.txt`: `Unhandled Exception Code 0xC0000005` tuz po `Body mesh name fetched: body_male_a` - w trakcie wczytywania PIERWSZEGO skinu rasy whitewalker (meski adult, tuz po ostatnim skinie ludzkim); CrashScribe: stan "outside a campaign - main menu or loading". Plik cofniety z .bak. Przyczyna (diff patched vs human): w blokach palet siedza ZAKOMENTOWANE fragmenty `<!-- <hair_color_gradient_point .../> ... -->` z dodatkowymi punktami; skrypt liczyl je jako prawdziwe, wycinal komentarz i generowal tyle REALNYCH punktow - wlosy 48 zamiast 24; natywny loader FaceGen ma sztywny bufor i pada z Access Violation. Poprawka skryptu: liczenie punktow po usunieciu komentarzy; kontrola: dla kazdego z 10 skinow liczba realnych punktow przed i po IDENTYCZNA (skora 24, wlosy 24, oczy 20; komentarzy w skinie 23-30). Wgrane ponownie 2026-09-17 po 15:50 (gra zamknieta; skins.xml md5 b12f717fd3ff41f0bd00e53eafc7dffd), .bak z oryginalem (89b17643e4872fd66f88176629bf1fd4) bez zmian.
**Status:** WGRANE 2026-09-17 (drugie podejscie, po crashu pierwszego). DO SPRAWDZENIA: gra ma sie wczytac do menu i do save bez AV; potem wyglad Innego.


## 2026-09-17 - "Buduje onager, nie postepuje ani o 1%": onager czeka, bo PRZYGOTOWANIA OBOZU wciaz w budowie (gra buduje machiny dopiero po nich); do tego dzienna diagnostyka oblezenia w logu, bo tempo 0.55%/h nie drgnelo po zmianie suwaka RB 0.4 -> 1.0
**Mod:** Armoury | **Pliki:** `Armoury/src/WorldPace.cs` (SiegeDaily)
**Dowod z logu (Armoury-2026-09-17_13-17-46):** `13:36:27 WorldPace: budowa preparations (24 osobodni, strona Attacker, zdrowych 300): gra 0.55%/h -> po suwaku 50% 0.28%/h` - w tej sesji (dzien 183080-183092) strona gracza buduje NADAL przygotowania obozu, onager nie jest nawet wywolywany. Vanilla `SiegeEvent.ConstructionTick` (dekompilacja): dopoki `SiegePreparations` nie sa gotowe, buduje sie TYLKO je; machiny z kolejki stoja na 0%. Do tego zagadka: to samo 0.55%/h co o 12:30, a `RealisticBannerlord_v2.json` ma od 13:09 `SiegeConstructionSpeedMultiplier = 1.0` (MCM go nie nadpisal, zakres suwaka 0.1-1.0) - wg kodu RB tempo atakujacego = vanilla x mnoznik x 0.75 (bez Tools) powinno skoczyc 2.5x. Z formuly vanilla (sqrt zdrowych / (osobodni x 24)) 0.55%/h przy mnozniku 0.4 wychodzi ~111 zdrowych, nie 300 - albo RB nie czyta MCM (kod: `GlobalSettings<RealisticSettings>.Instance?.X ?? 0.4f` - przy Instance == null bierze sztywne 0.4), albo gra liczy sile robocza z innej listy partii niz partia gracza. Bez logu nie rozstrzygne.
**Zmiana:** raz na dzien kampanii, strona atakujaca oblezenia gracza, linia `WorldPace: oblezenie <osada> dzien N: przygotowania obozu P% (W BUDOWIE - machiny czekaja | gotowe); machiny w budowie: onager X%; sila robocza wg gry S (sqrt zdrowych) -> ~Z zdrowych; partie w obozie: <nazwa> zdrowi/wszyscy, ...; Tools w sakwach gracza T; RB mnoznik budowy z MCM = M | (Instance null -> RB bierze 0.4)`. Zero zmian w mechanice.
**Ryzyko / co sprawdzic:** po wczytaniu w oblezeniu jedna linia dziennie; z niej wynika (a) ile jeszcze przygotowan, (b) czy RB widzi 1.0, (c) ilu zdrowych liczy gra. Jesli `Instance null` - RB ignoruje swoj MCM i mnoznik trzeba wymusic patchem (Mends); jesli sila robocza ~111 przy 300 ludziach - problem lezy w liscie partii obozu.
**Status:** WGRANE 2026-09-17 (Armoury.dll md5 f62bbb492b97c6d63b9193e85f251413, build rc=0, gra zamknieta; poprzednie DLL obok jako `Armoury.dll.bak-2026-09-17-przed-diag-oblezenia`). DO SPRAWDZENIA w logu po wczytaniu.


## 2026-09-17 - "Ciagle widze tylko trzy typy kryjowek": nakladka ROT_Hideouts od zawsze byla pustym przebiegiem (celowala w Hideout/@scene_name, mapa trzyma scene w Location) - poprawiony plik xslt, 266 kryjowek dostaje 22 sceny zamiast 3-4
**Mod:** patch cudzego modu (ROT_Hideouts v1.2.11) | **Pliki:** `patches/ROT_Hideouts/settlements.xslt` (NOWY, kopia wgrana do `Modules\ROT_Hideouts\ModuleData\settlements.xslt`)
**Zgloszenie (Jeff 17.09):** "ciagle sa te same lokalizacje w hideout, widze tylko trzy typy w kolko, sprawdz to dokladnie" (i: nie losowac scen, bo nadmorska nie moze byc pustynna).
**Dowod:** log silnika `ProgramData\...\logsgl_log_6572.txt` (sesja 12:29): ladowane sceny kryjowek `sea_bandit_a_sv` x4, `mountain_hideout_002_sv` x3. Wg plikow po nakladce ROT_Hideouts ZADNA kryjowka nie powinna miec `sea_bandit_a`. Test nakladki dokladnie tym samym silnikiem co gra (`System.Xml.Xsl.XslCompiledTransform`, PowerShell) na `ROT-Map/ModuleData/settlements.xml`: 266 kryjowek, po transformacji sceny IDENTYCZNE jak przed (46 forest_hideout_004_sv, 45 bandit_forest_sv, 30 mountain_hideout_002_sv, 18 sea_bandit_a_sv ...). Przyczyna: szablony xslt to `match="Hideout[@id='hideout_forest_1']/@scene_name"`, a mapa ROT (`<Hideout id=... map_icon=... />` bez scene_name; scena w `<Locations><Location id="hideout_center" scene_name="sea_bandit_a_sv"/>`) - zaden z 259 szablonow nie trafia. Silnik gry (dekompilacja `MBObjectManager.GetMergedXmlForManaged`/`CreateMergedXmlFile`) sam xslt bez xml obsluguje poprawnie, a `Settlement.Deserialize` czyta `scene_name` z XML przy KAZDYM wczytaniu (save nie zamraza scen) - wiec poprawka dziala od nastepnego wczytania. Wniosek uboczny: 12 scen z ROT_Hideouts NIGDY nie zaladowalo sie w tej grze; folder `Scene Design Contest 2 - Hideouts` jest zagniezdzony (nie jest modulem), ale jego 3 sceny sa juz w ROT_Hideouts.
**Zmiana:** wygenerowany nowy xslt z TYMI SAMYMI przypisaniami autora (236 szablonow), cel `Settlement[@id]/Locations/Location[@id='hideout_center']/@scene_name`; sceny vanilla w wersji `_sv` (jak mapa); 16 przypisan do scen, ktorych nie ma w zadnym module (`forest_hideout_003` bez `_sv`, `desert_hideout_003`, `hideout_mountain_3`, `hideout_steppe_8`), pominietych - te kryjowki zostaja jak na mapie; 7 id spoza mapy (seaside_53/68, wight_hideout_*) pominiete. Test po poprawce: 266 kryjowek, 22 rozne sceny; okolice The Rills: mountain_24 -> Scene_Levente, seaside_24 -> scn_smugglersdescent1, forest_40 -> Scene_Ceftadzime, seaside_23 -> RuinSeaside, mountain_25 -> Scene_Ceftadzime, seaside_21/25 -> sea_bandit_c_sv; liczba osad 1065 przed i po (miasta/zamki/wioski nietkniete). Przypisania autora sa terenowe (sea_raiders -> smugglers descent/shipwreck cove/RuinSeaside/OldVlandiaTown, mountain -> Levente/Ceftadzime/Canyon, desert -> patatest/desert), z paroma dziwactwami autora (desert_bandits -> contest_shipwreck_cove x4).
**Ryzyko / co sprawdzic:** 12 scen z ROT_Hideouts (pisane pod 1.2.x) NIGDY nie chodzily w tej grze (1.4.x) - wejscie do kryjowki z taka scena moze sie wysypac albo miec braki (spawny, boss). Test: wejsc do jednej kryjowki z nowa scena (np. hideout_mountain_24 przy The Rills, Scene_Levente) i sprawdzic w `rgl_log` nazwe sceny oraz czy misja dziala. Cofniecie: przywrocic `settlements.xslt.bak-2026-09-17-stary-format`. Zmiana dziala od nastepnego wczytania save.
**Status:** WGRANE 2026-09-17 (md5 8df6eb7175825676d18435b8e3712019 w repo i w grze, gra zamknieta; oryginal obok jako `settlements.xslt.bak-2026-09-17-stary-format`). DO SPRAWDZENIA po wejsciu do kryjowki.
...
**Dowod z logu (Armoury-2026-09-17_11-36-10, 11:36:48):** `WorldPace: mapa 50% (4 modeli), budowa oblezen 50% (2 modeli)` - nasz SiegePostfix siedzi na DWOCH modelach: `DefaultSiegeEventModel` i `RealisticBannerlord.Systems.Siege.RealisticSiegeEventModel`. Dekompilacja RB (`scratchpad/rb/RealisticSiegeEventModel.cs`): `GetConstructionProgressPerHour` WOLA bazowy Default (juz z naszym x0.5), mnozy przez MCM `SiegeConstructionSpeedMultiplier` (json Jeffa: 0.4), a oblegajacy bez `Tools` w sakwach zadnej partii oblegajacej dostaje jeszcze x0.75; potem nasz postfix na metodzie RB dokladal x0.5 DRUGI raz. Razem: 0.5 x 0.4 x 0.75 x 0.5 = 0.075 tempa gry (z Tools: 0.1). Ten sam blad, co 02.09 przy predkosci i morale (SpeedDepth) - tam byl juz licznik zagniezdzenia, budowy machin nie obejmowal.
**Regula gry (vanilla `DefaultSiegeEventModel.GetConstructionProgressPerHour`):** postep/h = 1 / (osobodni machiny / sqrt(zdrowych po stronie) x 24), plus bonus z Inzynierii inzyniera partii i perki. Koszty (`Native/ModuleData/siegeengines.xml`): przygotowania obozu 48, taran 12, ulepszony taran 16, wieza 16/24, balista 8, katapulta 16, onager 20, trebusz 24. Dla 300 zdrowych (sqrt = 17.3): taran vanilla ~17 h, przygotowania ~66 h (2.8 dnia). Przy 0.075: taran ~9 dni, przygotowania ~37 dni. Dzien kampanii to nadal 24 h - nasz kalendarz (`Calendar.cs`) wydluza tylko rok do 168 dni (6 tygodni w sezonie), dni nie podwaja. Banner Kings: tylko +20% z perku Siege Overseer. RB `SiegeExpansionBehavior` zjada oblegajacemu 2 hardwood + 1 tools dziennie (bez nich komunikat "engines falling into disrepair", ale `DamageSiegeEngines` jest PUSTE - bez skutku).
...
**Sprawdzone w logu (Armoury-2026-09-17_12-29-38):** `12:29:48 SpeedDepth: ... budowa machin 2 modeli`; `12:30:42 WorldPace: budowa preparations (24 osobodni, strona Attacker, zdrowych 300): gra (z innymi modami) 0.55%/h -> po suwaku 50% 0.28%/h = ~363 h (15.1 dnia) od zera` - suwak liczy sie dokladnie raz (0.55 -> 0.28). Jeff jest wciaz na PRZYGOTOWANIACH obozu (nie na taranie): przy RB 0.4 x 0.75 (bez Tools) i naszych 50% to 15 dni; sam RB daje 0.55%/h zamiast vanillowych ~3%/h. Uwaga: 0.55 to mniej niz 3.0 x 0.3 = 0.9 z mojego rachunku - gra liczy do osobodni mniej niz 300 zdrowych albo jest jeszcze jeden czynnik; do wyjasnienia osobnym logiem `GetAvailableManDayPower` (nie zrobione). Strona obroncy: `fire_catapult ... 1.61%/h -> 0.80%/h (5.2 dnia)` - obronca u RB ma x1.25 i bez kary za Tools; liczba `zdrowych` w tej linii to zawsze partia gracza (kosmetyka logu).
**Status:** WGRANE 2026-09-17 i SPRAWDZONE w logu 12:30 (Armoury.dll md5 998cf47667657a448c94ca0c77ef38f7, build rc=0, gra zamknieta; poprzednie DLL obok jako `Armoury.dll.bak-2026-09-17-przed-tempem-oblezen`).
...
**Sprawdzone w logu (Armoury-2026-09-17_11-36-10 i 12-29-38):** `DesertionLaw: model dodany - progi morale t1<25 t2<22 t3<19 t4<16 t5<13 t6<10 t7<10`, `czynny model dezercji = Armoury.TierDesertionModel`, `partia gracza 300 ludzi, limit 300, morale 100, glod: nie, OBLEGA`; partia klanu `Hodor's Party morale 23 - dezercja 1 (morale 1): Tully Recruit x1 (t1, prog 25)` i `morale 20 ... Northern Recruit x1` - prawo dziala na partie klanu gracza. CrashScribe: `Mends: oboz gracza pod The Rills nakarmiony (morale 100, 300 ludzi) - bez strat RealisticBannerlord` codziennie.
**Status:** WGRANE 2026-09-17 i SPRAWDZONE w logu (Armoury.dll md5 975fa127378e4f74e2642d2637afed5f -> od 12:29 998cf47667657a448c94ca0c77ef38f7 z tempem oblezen; poprzednie DLL obok jako `Armoury.dll.bak-2026-09-17-progi-80`).
...
**Dowod:** w naszych logach oblezenia nie ma (Armoury/CrashScribe nie loguja oblezen ani ubytkow - od dzis loguja), wiec przeszedlem po kodzie WSZYSTKIEGO, co zabiera ludzi partii oblegajacej (dekompilacja 17.09, `scratchpad/tw`, `scratchpad/rb`):
1. **RealisticBannerlord v1.2.0, `Systems.Siege.SiegeAttritionBehavior.OnDailyTickParty`** - kazdej partii z `BesiegerCamp` co dzien usuwa `RandomInt(SiegeAttritionMin, SiegeAttritionMax+1)` szeregowych, +1 za kazde 5 dni oblezenia (do +10), chirurg Medicine>=100 polowi, >=50 x0.75, MINIMUM 1; MCM Jeffa (`Configs\ModSettings\Global\RealisticBannerlord\RealisticBannerlord_v2.json`): `SiegeAttritionEnabled=True, SiegeAttritionMin=2, SiegeAttritionMax=5`. Zero zwiazku z jedzeniem i morale. Komunikat w grze: "Your siege camp suffered N casualties (day X of siege - attrition is escalating!)". To jest "trace codziennie wojsko" przy pelnym brzuchu i morale 100. (Garnizon oblezony: 1-2 dziennie, +1-2 bez zapasow.)
2. **Vanilla dezercja (`DefaultPartyDesertionModel`)**: z morale dopiero PONIZEJ 10 (jeden prog dla wszystkich, szansa wedle poziomu); z zoldu ponad limit wyplat (do 20 dziennie); z przepelnienia partii ponad `PartySizeLimit` (25% nadwyzki dziennie, min. 1). Przy morale 100 i partii w limicie - zero.
3. **Vanilla ostrzal (`SiegeEventCampaignBehavior`)**: kazde trafienie balisty/katapulty obroncy w NASZA machine z szansa `AntiPersonnelHitChance` zabija 1 (chirurg: `PartyHealingModel.GetSiegeBombardmentHitSurgeryChance` zamienia w rannego), zniszczona machina = 2 zabitych. To realne straty pod ogniem - ZOSTAJA.
4. RealisticBannerlord `RealisticPartyMoraleModel`: zima -15 morale, zamiec -20, burza wiosenna -10 (sezon i pogoda) - moze zbic morale w zimowym oblezeniu; `SiegeExpansionBehavior` zjada hardwood/tools na machiny. Banner Kings, ROT, BKROTPatch, BetterEconomy (BEE_PartySizeLimitModel - limit partii), Diplomacy: bez wlasnej dezercji ani strat oblezniczych (grep po dekompilatach: zero trafien).
**Zmiana:**
1. **CrashScribe Mends - oboz nie gnije, gdy jest nakarmiony:** prefix na `SiegeAttritionBehavior.OnDailyTickParty`: straty RB TYLKO gdy partia GLODUJE (`Party.IsStarving`) - wtedy robia za choroby obozowe; prefix na `OnDailyTickSettlement`: garnizon traci tylko, gdy miasto ma `FoodStocks <= 0`. Klucz `RbSiegeAttritionOnlyStarving` (settings.xml, domyslnie true; false = RB po staremu). Log przy starcie `Mends: oboz oblezniczy nie gnije, gdy jest nakarmiony ...` i co dzien oblezenia gracza `Mends: oboz gracza pod X nakarmiony (morale M, N ludzi) - bez strat RealisticBannerlord.`
2. **Armoury - prawo dezercji po tierach (`TierDesertionModel : DefaultPartyDesertionModel`, `starter.AddModel`):** kazdy tier ma wlasny prog morale: t1 `DesertionMoraleTier1` (80), kazdy tier wyzej o `DesertionMoraleStepPerTier` (5) nizej, nie ponizej `DesertionMoraleFloor` (40): t1<80 t2<75 t3<70 t4<65 t5<60 t6<55 t7<50. Ponizej progu ze stosu odchodzi dziennie `DesertionPercentPerMoralePoint` (1%) za kazdy punkt morale ponizej progu, sufit `DesertionDailyCapPercent` (25%); ranni i zdrowi proporcjonalnie; bohaterowie nigdy. Rekruci t1 przy morale 60: 20% dziennie; elita t6 przy 50: 5%. Glod i brak zoldu dzialaja przez morale (kary vanilla) i przez te progi; zold ponad limit wyplat i przepelnienie partii - wprost z vanilla (prywatna `GetTroopsToDesertDueToWageAndPartySize` przez refleksje). Prawo obejmuje partie gracza i jego klanu; AI zostaje przy vanilla (`DesertionLawForAi=false`) - lordowie AI chodza z morale 50-70 i wykrwawialiby sie z rekrutow. `GetMoraleThresholdForTroopDesertion` zostaje 10 (czyta je bazowa formula AI). Log: `DesertionLaw: <partia> morale M - dezercja N (morale a, zold/limit partii b): Oddzial xN (tK, prog P)`; komunikat gracza "N men slipped away in the night - morale M is below their threshold (...)". Przy wczytaniu: `DesertionLaw: czynny model dezercji = Armoury.TierDesertionModel (nasz; progi ...)` i `DesertionLaw: partia gracza N ludzi, limit L, morale M, glod: nie[, OBLEGA]`.
**Ryzyko / co sprawdzic:** w CrashScribe przy starcie linia `Mends: oboz oblezniczy nie gnije...` (bez `[NIEZNALEZIONE]`); w Armoury przy wczytaniu `czynny model dezercji = Armoury.TierDesertionModel` - jesli inny, to ktorys mod dodaje swoj model po nas i prawo nie dziala; w oblezeniu co dzien `Mends: oboz gracza pod ... bez strat RealisticBannerlord` i BRAK komunikatu "Your siege camp suffered". Straty z balist obroncy (pkt 3) zostaja - to nie dezercja. Jesli partia jest ponad limit (`limit L` w logu mniejszy niz ludzi) - vanilla nadal odsyla 25% nadwyzki dziennie (min. 1) - to zold/limit, nie morale. Partie klanu gracza pod tym prawem: przy niskim morale (np. glod w polu) rekruci zaczna odchodzic juz przy 80, nie przy 10 - to zamierzone.
**Status:** WGRANE 2026-09-17 (Armoury.dll md5 5f21b575d5e4c79fdb24095ba5b7a60d, CrashScribe.dll md5 f2eaef4cb71c5bbf648193baf610e6aa, oba build rc=0, gra zamknieta; poprzednie DLL obok jako `.bak-2026-09-17-przed-dezercja`; `Modules\CrashScribe\ModuleData\CrashScribe.settings.xml` podmieniony z nowym kluczem, kopia `.bak-2026-09-17-przed-dezercja`, XML sprawdzony parserem). DO SPRAWDZENIA po wczytaniu i w oblezeniu.


## 2026-09-16 - "QM: 52 empty battle bow": rozkaz z ksiegi musztry na luk trafial OBOK luku wzorca (dwa luki, jeden kolczan na czlowieka) - rozkaz zastepuje luk wzorca, wolne sloty dopelnia amunicja i bron boczna
**Mod:** Armoury | **Pliki:** `Armoury/src/SkillsDecide.cs` (RearmBySkill - rozkazy w slotach broni)
**Zgloszenie (Jeff 16.09):** "mam QM: 52 empty battle bow" (po rozkazach w ksiedze musztry: Riverlands Ranger / Mallister Elite Archer / Tully Longbowman -> Weapon 2 = weirwood_bow, Bolton Hunter -> Weapon 2 = ravens_teeth_longbow, Weapon 3/4 = ravens_teeth_arrows).
**Dowod z logu (CrashScribe session-2026-09-16_12-11-54, 12:14:15 -> 12:30:34):** `Mends: PUSTE Bow u gracza: 8x Tully Longbowman 150, 28x Riverlands Ranger 140, 16x Mallister Elite Archer 140 | podaz grupy: 119 szt., Difficulty 140-175 (noszone + pula DTE)` i `swieta zasada skilli w DTE [Ithauser's Party, 2225 slotow]: ... 44 luk zapelnionych z polki, 52 slotow PUSTYCH - nic uzytecznego na polce (Bow 52)`. Puste sloty to DOKLADNIE wszyscy ludzie trzech oddzialow z rozkazem na Weirwood (Bolton Hunter z rozkazem na Ravens Teeth nigdy na liscie - Ravens jest pod dostatkiem). Papier kwatermistrza z tej samej minuty (Armoury 12:29:10): `papier Bow: potrzeba 111, polka 119, udzwigna 111, bez sztuki 0` - papier liczy JEDEN luk na czlowieka i braku nie widzi. Linie `PUSTE Bow` w sesjach 04:12-09:39: zero; pierwsza o 11:24:26 (sesja 10-34, x21) - dokladnie wtedy, gdy pojawily sie rozkazy na Weirwood (wpis wyzej).
**Przyczyna:** ksiega musztry pokazuje sloty z szablonu XML jednostki (`OpenTroop`: `ch.BattleEquipments`; river_ranger: Weapon 1 = vlandia_sword_3_t4, Weapon 2 = lowland_longbow, Weapon 3/4 = bodkin_arrows_a), ale wzorzec bitewny buduje `SkillsDecide.RearmBySkill` po skillach: [luk, kolczan, kolczan, miecz]. Rozkaz "Weapon 2 = weirwood_bow" wchodzil wiec w slot KOLCZANU, obok luku wzorca: [luk wzorca, weirwood, strzaly, miecz] - dwa luki i jeden kolczan. Straz skilli (CrashScribe SkillLawWard) tworzy popyt na kazdy luk ze wzorca - drugi nie ma pokrycia (111 lukow na 111 ludzi) i wychodzi "PUSTY", choc czlowiek luk ma; DTE przy nadmiarze wydawal dwa luki jednemu (Bolton Hunter: 2x Ravens Teeth), a lucznicy z rozkazem szli do bitwy z jednym kolczanem zamiast dwoch.
**Zmiana (`RearmBySkill`, tylko gdy jednostka ma jakis rozkaz w slotach 0-3):** rozkaz na luk/kusze czysci inny, nie-rozkazany luk/kusze we wzorcu (rozkaz ZASTEPUJE luk wzorca, nie doklada); potem wolne sloty dopelniane amunicja do DWOCH kolczanow/sajdakow (dotad tylko gdy nie bylo zadnego), a gdy po tym zostaje wolny slot i nie ma zadnej broni bialej - bron boczna wzorca (`second`). Riverlands Ranger z rozkazem: [strzaly, weirwood, strzaly, miecz]; Bolton Hunter z trzema rozkazami: [miecz, ravens, rt_arrows, rt_arrows]. Bez rozkazu nic sie nie zmienia (`anyPin`).
**Poprawka do wpisu wyzej (rozkazy):** uwaga "Bolton Hunter ma Bow ponizej 175, rozkaz pomijany" byla BLEDNA - XML ROT (bolton_master_archer) ma Bow 140, ale w grze Jeffa Bolton Hunter ma 175 (histogram papieru: 3x175; Boltoni nigdy na liscie PUSTE), wiec rozkaz na Ravens Teeth dziala. 7 Ravens Teeth na liscie gracza to nadwyzka ponad 31 ludzi ze skillem 175+.
**Pytanie Jeffa "czemu nie oddali poprzednich lukow do stashu":** oddali - papier 11:44:42 (Armoury 11-43-22): na liscie gracza `training_longbow 4+5+2, woodland_longbow 14+6, training_bow 15, lowland_yew_bow 4` (porzadek Bow +50), a papier 11:45:04 (po zamknieciu ekranu) juz ich na polce nie ma (polka 168 -> 118) - zabrane w tej sesji ekranu do sakw gracza (z polki widocznej w stashu znika tylko to, co gracz sam przeniesie; wymiana barterowa tez wydaje do sakw).
**Ryzyko / co sprawdzic:** w bitwie / po wczytaniu w CrashScribe: `PUSTE Bow u gracza` ma zniknac (albo zostac tylko realny brak), `swieta zasada ... 0 slotow PUSTYCH`; HIT `od river_ranger [weirwood_bow]`. Ksiega musztry nadal POKAZUJE sloty z XML, nie zbudowany wzorzec - do osobnej decyzji (pokazywac wzorzec z SkillsDecide).
**Status:** WGRANE 2026-09-16 (Armoury.dll md5 169bbdf61f11f6e8222231f77469ec27, build rc=0, gra zamknieta; poprzednie DLL obok jako `Armoury.dll.bak-2026-09-16-przed-podwojnym-lukiem`, md5 15218077b6d41e097acae86e658600ff). Pierwsza proba 12:4x odrzucona przez system - DLL zablokowany przez dzialajaca gre (proces `Bannerlord.BLSE.LauncherEx`). DO SPRAWDZENIA w logu CrashScribe po bitwie / wczytaniu.


## 2026-09-16 - "Wrzucilem 54 Weirwood i 38 Ravens Teeth, zamknalem i otworzylem stash - nie zniknely": rozkaz z ksiegi musztry zamrazal ksiege wkladow, noszone sztuki z rozkazem zostawaly "gracza" i escrow pokazywal je dalej
**Mod:** Armoury | **Pliki:** `Armoury/src/QuartermasterLaw.cs` (PurgeUnusable), `Armoury/src/MusterBook.cs` (DescribePins)
**Zgloszenie (Jeff 16.09):** "wrzucilem w stash Weirwood Bow 54 sztuki oraz 38 Ravens Teeth i zamknalem i otworzylem stash i nie zniknely, nie wymienilem lukow" (screen: 170 Pound Weirwood Bow x54, 200 Pound Ravens' Teeth Longbow x38 dalej w stashu).
**Dowod z logu (Armoury-2026-09-16_11-43-22, 11:44:42 / 11:45:04 / 11:46:30):** papier Bow mowi, ze ludzie je biora: `ravens_teeth_longbow ... uzyte=31/38 (gracza 38)`, `weirwood_bow ... uzyte=54/54 (gracza 54)` - a `(gracza N)` to ksiega wkladow: sztuki NOSZONE nadal zapisane na gracza. Porzadek Bow z tego samego otwarcia ma SAME plusy: `training_longbow +11, woodland_longbow +20, training_bow +15, lowland_yew_bow +4` (wyparte gorsze luki na liste gracza), zero `weirwood_bow -54` / `ravens_teeth_longbow -31`. Przy zamknieciu 11:45:02 i otwarciu 11:46:30 - to samo, dalej `(gracza 54)` / `(gracza 38)`. Escrow pokazuje w stashu dokladnie ksiege gracza (`na liscie gracza: ... ravens_teeth_longbow x38 ...`), wiec luki "nie znikaja". Wczesniej tego dnia te same id ksiega ruszala normalnie: `05:48:51 weirwood_bow +1`, `10:30:50 ravens_teeth_longbow -1`, `10:42:32 ravens_teeth_longbow -2`, `11:22:54 ravens_teeth_longbow +2`; od 11:24:00 (log 10-35) `weirwood_bow uzyte=35/54 (gracza 54)` przy kazdym otwarciu bez korekty, od 11:44:42 to samo z Ravens Teeth.
**Przyczyna:** w `PurgeUnusable` jedyne miejsce, ktore pomija pozycje ksiegi, to `if (MusterBook.IsPinnedItem(id)) continue;` ("rozkaz z ksiegi swiety", wpis z 14.09). Rozkaz w ksiedze musztry (Assign w "Open the muster book") powstaje TYLKO z klikniecia gracza - Jeff przypisal te luki swoim lucznikom (Weirwood przed 11:24, Ravens Teeth miedzy 11:22 a 11:44), zapewne zeby wymusic ich wydawanie. Skutek uboczny: cala pozycja z rozkazem wypadala z porzadku, wiec 54+38 wkladu gracza nigdy nie schodzilo na stan wojska, choc papier liczyl je jako noszone; escrow (lista = ksiega gracza) pokazywal je dalej, a wyparte gorsze luki dochodzily obok (+50). Wyglada to jak "nikt nie wzial, a dosypal mi smieci". (Sam wybor lukow po naciagu - wpis nizej - dziala: 54/54 i 31/38 na papierze.)
**Zmiana:** (1) `PurgeUnusable`: pozycja z rozkazem NIE jest juz pomijana w calosci - sztuki noszone (delta ujemna) przechodza na stan wojska jak kazde inne i znikaja z listy gracza; rozkaz chroni tylko sztuki nienoszone przed oddaniem na liste gracza (czekaja w magazynie na ludzi z rozkazem) i jak dawniej przed przycinaniem (`TrimWarStores` bez zmian). (2) Dowod w logu: przy kazdym porzadku linia `Kwatermistrz: rozkazy z ksiegi musztry: <jednostka>|<slot>=<item>, ...` (nowe `MusterBook.DescribePins`) oraz `Kwatermistrz: rozkazy - noszone z rozkazem na stan wojska: <id>...; nienoszone z rozkazem zostaja na stanie: N szt.`. Dotad rozkazy nie zostawialy w logu zadnego sladu.
**Ryzyko / co sprawdzic:** po otwarciu zbrojowni: linia `rozkazy z ksiegi musztry:` z weirwood_bow / ravens_teeth_longbow (potwierdzenie hipotezy - jesli jej NIE ma, przyczyna jest inna i wpis do poprawy), `porzadek Bow: weirwood_bow -54, ravens_teeth_longbow -31`, papier `weirwood_bow uzyte=54/54` BEZ `(gracza 54)`, w stashu Weirwood znika, Ravens Teeth zostaje 7 (dla 31 ludzi ze skillem 175+ z 38 sztuk). Cofniecie rozkazu ("back to the company pattern") oddaje nienoszone na liste gracza przy nastepnym porzadku. Jesli Jeff NIE przypisywal lukow w ksiedze - linia rozkazow to pokaze i szukamy dalej.
**Sprawdzone w logu (Armoury-2026-09-16_12-11-55, 12:13:07, zero ERROR):** `rozkazy z ksiegi musztry: Riverlands Ranger|Weapon 2=weirwood_bow, Mallister Elite Archer|Weapon 2=weirwood_bow, Tully Longbowman|Weapon 2=weirwood_bow, Bolton Hunter|Weapon 2=ravens_teeth_longbow, Bolton Hunter|Weapon 3=ravens_teeth_arrows, Bolton Hunter|Weapon 4=ravens_teeth_arrows` - hipoteza potwierdzona (Jeff przypisal luki w ksiedze). `porzadek Bow: ravens_teeth_longbow -31, weirwood_bow -54`; papier `weirwood_bow uzyte=54/54` bez `(gracza)`, `ravens_teeth_longbow uzyte=31/38 (gracza 7)`; `na liscie gracza: ... ravens_teeth_longbow x7`. Uwaga: rozkaz Bolton Hunter -> Ravens Teeth (wymog 175) jest przy wzorcu pomijany, bo Bolton Hunter ma Bow ponizej 175 (`ItemReq.Meets`); 7 sztuk zostaje na liscie gracza, bo tylko 31 ludzi ma 175+.
**Status:** WGRANE 2026-09-16 12:05 i SPRAWDZONE w logu 12:13 (Armoury.dll md5 15218077b6d41e097acae86e658600ff, build rc=0, gra zamknieta przy wgraniu; poprzednie DLL obok jako `Armoury.dll.bak-2026-09-16-przed-rozkazami`).


## 2026-09-16 - "Lucznicy nie biora Weirwood" ROZSTRZYGNIETE: ranking lukow po naciagu RBM z runtime zamiast starej skutecznosci z XML (kwatermistrz, straz skilli, wzorce)
**Mod:** Armoury + CrashScribe | **Pliki:** `Armoury/src/RangedRank.cs` (NOWY), `Armoury/src/QuartermasterLaw.cs` (FitFor sort, Detail), `Armoury/src/SkillsDecide.cs` (AddPattern), `CrashScribe/src/Mends.cs` (RangedKey, SkillLawWard sort)
**Zgloszenie (Jeff 16.09):** "mam lucznikow co maja bow 140 150 170, ale nie chca brac tych lukow (54x 170 Pound Weirwood Bow, 38x 200 Pound Ravens Teeth Longbow w stashu), wchodze i wychodze i nie zamieniaja".
**Dowod z logu (Armoury-2026-09-16_10-35-01, 11:30:47, nowy papier Bow):** `potrzeba 111, polka 168, udzwigna 111, bez sztuki 0. Skill Bow ludzi: 36x140 44x150 3x175 2x230 26x260` i kolejnosc wyboru: `training_longbow wymog=175 skut=609 t6 uzyte=11/11, woodland_longbow wymog=175 skut=537 uzyte=20/20, ravens_teeth_longbow wymog=175 skut=96 uzyte=0/38, training_bow wymog=140 skut=515 uzyte=36/36, nomad_bow 512 3/3, lowland_yew_bow 486 4/4, weirwood_bow wymog=140 skut=91 uzyte=35/54`. Czyli 31 najlepszych strzelcow (175-260) dostalo TRAINING LONGBOW i woodland zamiast Ravens Teeth, a Weirwood szedl dopiero, gdy skonczyly sie training bow, nomad i yew.
**Przyczyna (dekompilacja ROT i RBMCombat):**
1. `ROT.CampaignBehaviors.ROTRBMCompatibility.ModifyBowsAndArrows` przy zaladowanym RBM nadpisuje w locie (refleksja na WeaponComponentData) statystyki lukow ROT: weirwood_bow MissileSpeed 93 -> 170, ThrustDamage 84, nazwa "170 Pound Weirwood Bow", Value 90000; ravens_teeth_longbow 87 -> 210, dmg 96, "200 Pound Ravens Teeth Longbow", 150000; dragonbone 180, goldenheart 200, giant bow 500.
2. `RBMCombat.RangedRework` bierze `CurrentUsageItem.MissileSpeed` jako NACIAG w funtach i z niego liczy prawdziwa predkosc strzaly (`Utilities.calculateMissileSpeed`); RBM-owe luki: training_longbow 200 lb (!), noble_long_bow 200, woodland 180, nomad 170, training_bow 166, lowland_yew 160 ... hunting_bow 60. Pod RBM Ravens Teeth (210) to najmocniejszy luk w grze, Weirwood (170) = nomad, powyzej yew i training_bow.
3. Vanillowe `ItemObject.Effectiveness` gra liczy RAZ przy wczytaniu z XML (Weirwood: missile 93, Ravens 87 -> skut 91/96), ZANIM ROT nadpisze statystyki - i wlasnie po tej stechlej liczbie sortowaly przy rownym wymogu: papier kwatermistrza (FitFor), straz skilli (SkillLawWard), wzorce klas (SkillsDecide.AddPattern). Stad training_longbow (skut 609) przed Ravens Teeth (96).
4. Wymog: prawo tieru broni daje Weirwood 140 (t5), Ravens Teeth 175 (t6) - zostaje. RBM za skill ponizej naciagu +9 daje tylko flage UnloadWhenSheathed (ponowne nalozenie strzaly po schowaniu), nie zakaz.
**Zmiana:** `RangedRank.Key(item)`: gdy RBM zaladowany i item to luk/kusza - klucz = `PrimaryWeapon.MissileSpeed` (naciag z runtime) x 1e6 + `ThrustDamage` x 1e3 + stara skutecznosc; inaczej stara skutecznosc. Uzyte w trzech miejscach wyzej (Mends ma wlasna kopie `RangedKey` - CrashScribe nie referencuje Armoury; trzymac w zgodzie). Wymog nadal pierwszy. Papier Bow pokazuje teraz `naciag=170lb dmg=84` zamiast `skut=`.
**Spodziewany efekt przy tej samej polce:** 175-260 (31 ludzi): Ravens Teeth 210 lb (38 szt.) przed training/noble longbow 200 i woodland 180; 140-150 (80 ludzi): nomad 170 (3), Weirwood 170 (54), potem training_bow 166, yew 160. Training longbow schodzi z elity.
**Ryzyko / co sprawdzic:** po otwarciu zbrojowni linia `papier Bow po kolei wyboru (wymog malejaco, potem naciag RBM)` z `ravens_teeth_longbow ... uzyte=31/38` i `weirwood_bow ... uzyte=~51/54`; w bitwie HIT `od river_ranger [weirwood_bow]`. Kusze tez po naciagu (RBM: "500 Pound Doubler Belt Crossbow" = missile_speed 500). Bez RBM nic sie nie zmienia.
**Status:** WGRANE 2026-09-16 (Armoury.dll md5 ff4c159ff8789fc18294cafbf6738b0a, CrashScribe.dll md5 a2972da07c6bcf637461ed82bafdedfe, repo i gra zgodne; poprzednie DLL obok jako `.bak-2026-09-16-przed-naciagiem`). SPRAWDZONE NA PAPIERZE 11:45:04 (Armoury-2026-09-16_11-43-22.log): `ravens_teeth_longbow wymog=175 naciag=210lb uzyte=31/38, nomad_bow 3/3, weirwood_bow wymog=140 naciag=170lb uzyte=54/54, training_bow 23/23`; porzadek 11:44:42 oddal graczowi 50 gorszych lukow (training_longbow +11, woodland_longbow +20, training_bow +15, lowland_yew_bow +4). Zero bledow. Bitwa (HIT `od river_ranger [weirwood_bow]`) do potwierdzenia.


## 2026-09-16 - Diagnostyka: kwatermistrz wypisuje "papier" takze dla broni (Jeff: "czemu lucznicy nie biora Weirwood?")
**Mod:** Armoury | **Pliki:** `Armoury/src/QuartermasterLaw.cs` (Fit.Detail/SkillHist, FitFor, ShortageLines)
**Zgloszenie (Jeff 16.09):** "mam 54 Weirwood Bow, wymog Bow 140, lucznicy tego nie uzywaja; jak wrzucam do stashu, luki sa zabierane, a w zamian dostaje inne".
**Co wiadomo z logow (bez zgadywania):** `weirwood_bow` w ROTassets.xml: difficulty 120, value 150000, culture battania, dmg 84 / speed 95 / accuracy 100 / missile 93; nasze prawo tieru broni (35 na tier) podnosi wymog do 140 (tier 5). RBM przepisuje vanillowe luki: woodland_longbow missile 180 (wymog 120), noble_long_bow 200 (140), training_longbow 200 (0!) - w swiecie RBM Weirwood strzela dwa razy wolniej niz zwykly longbow, wiec w `Effectiveness` gry (po ktorej papier i straz sortuja przy rownym wymogu) przegrywa. Papier kwatermistrza (FitFor) daje kazdemu strzelcowi NAJTRUDNIEJSZY luk, jaki udzwignie (wymog malejaco, potem skutecznosc) - zasada Jeffa z 14.09. Stare logi: 14-15.09 `porzadek Bow: training_bow -14, weirwood_bow +14`, `ravens_teeth_longbow -5, weirwood_bow +5`, `woodland_longbow -6, ... weirwood_bow +7` - Weirwood za kazdym razem wracal na liste gracza. Dzis (08:37) Jeff wlozyl 56 szt.; przy zamknieciu ekranu papier nie zrobil ZADNEJ korekty Bow (brak linii `porzadek Bow`), czyli nikt ich na papierze nie wzial; od tego czasu nie bylo bitwy (brak HIT w sesjach 08:09/08:32/09:39). Wymiana "wrzucam, dostaje inne" to reguly Jeffa z 29.08 (barter przy zamknieciu) i 14.09 (nadwyzki na liste gracza).
...
**Mod:** CrashScribe | **Pliki:** `CrashScribe/src/NightKingCall.cs` (NOWY), `CrashScribe/src/Config.cs`, `CrashScribe/src/SubModuleMain.cs`, `CrashScribe/ModuleData/CrashScribe.settings.xml` (5 nowych kluczy)
**Zgloszenie (Jeff 16.09):** "a jak nocny krol, bo nic sie nie dzieje". Wybral opcje 2 z trzech (tylko MCM / Zew / kopniak inwazji).
**Dowod z logu (session-2026-09-16_08-09-15, 08:20):** `UMARLI: Nocny Krol ZYJE (Night King) | Inni: 4 band, 668 trupow w polu, 0 osad | INWAZJA NA POLUDNIE: jeszcze nie` i `do ruszenia inwazji brakuje im 25 osad za Murem` - identycznie jak 30.08 (HISTORY.md: "0/25 osad od 594 dni"); dzien kampanii 840.
**Przyczyna (dekompilacja ROT.CampaignBehaviors.ROTOthersCampaignBehavior):**
1. inwazja rusza, gdy WSZYSTKIE osady za Murem naleza do Innych (`OnDailyTick`);
2. banda oblega TYLKO przy `TotalHealthyCount >= 500` (`OnAiHourlyTick`), a potem `(PartySizeRatio >= 0.8 || > 2000 ludzi) && 2.5x garnizon` albo `> 1000 && 4.5x`; rajd wioski (glowne zrodlo trupow) wymaga `> 400` ludzi;
3. przyrost: +2 trupy/dzien/banda (`OnDailyTickParty`) + nekromancja po wygranej (ograniczona wolnym miejscem w partii);
4. GWOZDZ: `OthersPartySizeLimitGrowth` (3/dzien) dolicza sie do `_partySizeGrowth`, ktore JEST W SAVE (`SyncData`) - po 840 dniach ~2500 miejsc premii do limitu kazdej bandy (`ROTPartySizeLimitModel`: "Undead Horde"), wiec `PartySizeRatio >= 0.8` jest nieosiagalne na zawsze, a bandy po 150-200 nie dobijaja ani do 400 (rajd), ani do 500 (oblezenie). Balans ROT zamrozil wlasna fabule.
**Zmiana (`NightKingCall`, raz na dzien kampanii, zadna stala ROT nie ruszona):**
1. jesli zadna banda Innych nie ma `NightKingCallTarget` (520) zdrowych: zdrowi NIE-bohaterowie z mniejszych band przechodza do najsilniejszej wolnej bandy (bandy w bitwie/oblezeniu tego dnia nie daja i nie biora; Nocny Krol gospodarzem tylko, gdy nie ma innej - ROT nie kaze mu oblegac zamkow); dawca zostawia sobie `NightKingCallKeep` (40), Nocny Krol `NightKingCallKeepNk` (120);
2. premia limitu (`_partySizeGrowth`) ustawiana tak, zeby najsilniejsza banda byla pelna w `NightKingCallFullness` (85%) swojego limitu - stan, jaki ROT zaklada dla oblezenia (>= 0.8), z 15% miejsca na nekromancje; baza limitu liczona z `PartySizeLimit - premia (- 250 u NK)`;
3. dalej decyduje ROT: cel (najbardziej polnocna twierdza za Murem), przewaga 2.5x, kajdany AI (Hardhome po dniu 500 - juz minelo), oblezenie, po 25 osadach inwazja, smiertelnosc Nocnego Krola (`NKMayBeExecuted`).
Log: `Zew Nocnego Krola: <lord> przyjmuje N trupow od M band -> X zdrowych z Y (cel 520, band w polu 4); premia limitu hordy 2520 -> P (baza B, limit L)`; gdy banda gotowa: `... ma X zdrowych - oblezenie w rekach ROT`. Wylacznik: `NightKingCallEnabled` w `CrashScribe.settings.xml`.
**Ryzyko / co sprawdzic:** pierwszego dnia pula to ~650 trupow minus co zostaje dawcom (~40x3 + 120) - gospodarz moze nie dobic do 520 od razu; od 400 ludzi ROT pozwala mu rajdowac wioski (Hearth/2 trupow za rajd), wiec prog powinien pasc w kilka dni. Po zbiciu premii limitu z ~2500 do kilkuset wszystkie bandy Innych maja limit wg bazy + premia - ich +2/dzien i nekromancja dzialaja dalej (miejsce jest). Przemarsz trupow jest "teleportem" miedzy bandami - swiadomie, to Zew. Jesli Wolni Ludzie rozbija wielka bande, ROT wskrzesza po swojemu (CanHeroDie=false przed inwazja). Sprawdzic w session log linie `Zew Nocnego Krola` i po kilku dniach `UMARLI: ... N osad`.
**Status:** WGRANE 2026-09-16 (CrashScribe.dll md5 7d1890eb4bc539bf11b259d31f00ab90, repo i gra zgodne; poprzedni DLL i settings.xml obok jako `.bak-2026-09-16-przed-zewem`). DO SPRAWDZENIA: linia `Zew Nocnego Krola` w session log, potem `UMARLI: ... N osad`.


## 2026-09-16 - Prawo unikatow, pierwsze wczytanie: 128 sztuk u gracza zamienione OK, ale 11134 sztuk w 337 magazynach AI dostalo ubrania BEZ kultury (MBObjectManager nie zna partii) - poprawka + jednorazowe przebranie
**Mod:** Armoury | **Pliki:** `Armoury/src/UniqueLaw.cs` (FindParty, RedressAiArmories, SyncData)
**Dowod z logu (Armoury-2026-09-16_08-09-16.log, 08:11:11):** `UniqueLaw (wczytanie): magazyn DTE gracza - 38 szt. zamienionych, 0 znikly; tabor gracza - 90 zamienionych` z zamiennikami w kulturze gracza (river): `ramsay_armor -> raven_teeth_armor (t6, river)`, `hound_gloves -> mallister_gloves2 (t5, river)`, `ramsay_helmet -> westerling_helmet3 (t6, river)`; bohaterowie: `Ithauser (river) zdejmuje brienne_helmet -> westerling_helmet3`, `... blackfyre_boots -> mallister_plated_boots2`, `... aemon_gauntlets -> mallister_gloves2`, `Hodor (battania) zdejmuje brienne_plate -> stark_knight_armor_2`. Zero ERROR. Wedrowcy-wlasciciele (Brienne, Sandor, Tyrion...) nietknieci.
ALE: `UniqueLaw: magazyny AI (wczytanie) - 11134 szt. unikatow zamienionych na zamienniki, 0 znikly, w 337 partiach` - wszystkie zamienniki z kultura `(-)`: `ramsay_armor (-) -> tacky_bandit (t5, neutral_culture)`, `ramsay_helmet (-) -> bandit_hybrid_armor_helmet_b (t3)`, `ramsay_boots (-) -> arryn_chausses (t5, vale)`. Skala: 11 tysiecy sztuk unikatow lezalo w magazynach AI - to byla ta fabryka (DTE dosypuje codziennie, patrz wpis wyzej).
**Przyczyna:** `MBObjectManager.Instance.GetObject(MBGUID)` NIE zna partii kampanii (zyja w `CampaignObjectManager`) - zwracal null, kultura partii = null, StandInFor bral bezkulturowe. DTE sam szuka partii przez `Campaign.Current.MobileParties.FirstOrDefault(p => p.Id == id)` (`FindActiveParty`). Ten sam blad siedzi od 14.09 w `LegendaryLaw.SweepAiArmories` (geografia wierzchowcow: `owner` zawsze null, wiec regula "mamut/wielblad nie u swoich" w magazynach AI nigdy nie dzialala) - NIE ruszane w tym wpisie, do osobnej decyzji.
**Zmiana:**
...
**Status:** WGRANE 2026-09-16 (Armoury.dll md5 5867afc34e27d704b578e301ccd38dce po dolozeniu sufitu kucia z wpisu nizej, repo i gra zgodne; poprzedni DLL obok jako `Armoury.dll.bak-2026-09-16-v1-unikaty`). DO SPRAWDZENIA po wczytaniu: linia `UniqueLaw: przebranie magazynow AI po pierwszym wczytaniu`.
...
**Mod:** Armoury | **Pliki:** `Armoury/src/Settings.cs` (+McmSettings), `Armoury/src/ArmouryBehavior.cs` (dwa miejsca z podloga czasu), `Configs\ModSettings\Global\Armoury\Armoury.json` (WeaponDaysPerTier)
**Zgloszenie (Jeff 16.09):** "kucie miecza jest za dlugie, zmniejsz jeszcze o 80% - czekam na cztery miecze ponad 169 godzin".
...
**Zmiana:** domyslne `WeaponDaysPerTier` 0.5 -> 0.1; podloga 0.5 -> 0.1 dnia (`ArmouryBehavior.AdvanceProjects` i start projektu broni); SUFIT Z SUWAKA: kazdy tick przycina `DaysLeft` zlecen broni JUZ W KOLEJCE do `max(0.1, tier * WeaponDaysPerTier)` (log `Kuznia: N zlecen broni w kolejce skrocono...`) - cztery czekajace miecze Jeffa skroca sie od razu, nie dopiero nowe zlecenia; `Armoury.json`: `WeaponDaysPerTier` 0.5 -> 0.1 (kopia `.bak-2026-09-16-kucie` przed zmiana, po zamknieciu gry). Miecz T4 = 0.4 dnia = ok. 10 godzin przy kowadle; cztery miecze = ok. 2 dni gry zamiast 7. Pancerze (`DaysPerTier` 1.0) bez zmian - Jeff mowil o mieczach.
...
**Status:** WGRANE 2026-09-16 (Armoury.dll md5 5867afc34e27d704b578e301ccd38dce, repo i gra zgodne - wgrany ponownie po dodaniu sufitu; `Armoury.json` WeaponDaysPerTier 0.5 -> 0.1, kopia `Armoury.json.bak-2026-09-16-kucie`, JSON sprawdzony). DO SPRAWDZENIA: po wczytaniu linia `Kuznia: 4 zlecen broni w kolejce skrocono...` i czas w kuzni.
...
**Mod:** Armoury | **Pliki:** `Armoury/src/UniqueLaw.cs` (NOWY), `Armoury/src/UniqueGear.cs` (zestaw Gory), `Armoury/src/MountLaw.cs` (IsEssosCulture), `Armoury/src/Settings.cs` + `McmSettings.cs` (UniqueGearLawEnabled), `Armoury/src/SubModuleMain.cs`
**Zgloszenie (Jeff 16.09):** "Ramsey Armour jest tylko jeden i nosi go Ramsey, teraz polowa wojska biega w Ramsey armour, plus biegaja z mieczami Dotrakow... pozamieniaj na odpowiednie historyczne i podobnej jakosci lub minimalnie mniejszej... meczace jest jak widze polowe wojska w pancerzu Clegana lub Mountain".
**Dowod z logu:** `Armoury-2026-09-15_14-14-08.log` 14:32: `Kwatermistrz: WYKLUCZONE z wydawania (59 szt., 15 id): ramsay_gloves x9, hound_gloves x7, ramsay_boots x4, ... ramsay_helmet x7, ramsay_armor x4, brienne_plate x1`; Armoury.log: `UniqueLore: nauczono ramsay_armor` (nauczony wzor = furtka strazy); CrashScribe 16.09 05:41: `Straz unikatow: 13 klng/unikatow zdjetych z przydzialu DTE, 202 sztuk pancerza cofnietych do wzorca` - po 200 cofniec na bitwe, czyli kopie zalewaly przydzial bez konca.
**Przyczyna (dekompilacja DTE 1.4.7 przez ilspycmd, 16.09):**
1. `EveryoneCampaignBehavior.AllocateRandomEquipmentToPartyArmory` co dzien dosypuje KAZDEJ partii AI losowy sprzet z `Cache.GetItemsByTypeTierAndCulture` - filtr tylko tier + `item.Culture == culture` (albo bez kultury). ROTassets.xml: `ramsay_armor culture=battania` (Polnoc), `hound_gloves`/`mountain_armor` `culture=vlandia` (Westerlands), `renly_armor`/`brienne_plate` stormlands. Kazda partia Polnocy dostawala wiec pancerz Ramsaya, kazda z Westerlands rekawice Ogara i zbroje Gory.
2. Po bitwie magazyn pokonanego jest lupem zwyciezcy (`GetAllLootItems -> AddItemToArmory` / `AddItemToPartyArmory`) - stad 9 par rekawic Ramsaya na polkach gracza. Straze Mends zdejmowaly je z przydzialu, ale kopie lezaly i wracaly kazdym lupem.
3. Zestaw Gory (`mountain_armor/helmet/pauldrons/boots/gloves/gauntlets`) nie byl na ZADNEJ liscie unikatow (prefiks `mountain_` odpada: `mountain_hunting_bow` to zwykly luk 20 kultur) - DTE rozdawal go bez przeszkod.
4. `ramsay_armor` byl NAUCZONY w kuzni (Armoury.UniqueLore), a UniqueWard z 30.08 przepuszczal nauczone unikaty do wojska - stad Ramsay na szeregowych (druga polowa tej naprawy: wpis CrashScribe ponizej).
5. Zwykle oddzialy NIE maja unikatow w szablonach (audyt 5833 postaci z XML ROT/SandBox): maja je tylko wedrowcy ROT, ktorzy SA wlascicielami (ROTwanderer3 = Sandor Clegane z hound_*, ROTwanderer9 = Brienne, 8 = Tyrion, 2 = Varys, 27 = Baelish, 7 = Gendry z bull_helmet). Szablonow wiec nie tykamy - to nie one mnozyly.
**Zmiana (UniqueLaw, wlacznik `UniqueGearLawEnabled` w MCM, domyslnie ON):**
1. NABOR: prefix na `ArmyArmory.AddItemToArmory` (magazyn gracza) i `EveryoneCampaignBehavior.AddItemToPartyArmory` (magazyny AI; tedy idzie i codzienny los DTE, i lup) - unikat w drzwiach zamienia sie w ZAMIENNIK: ten sam typ (bron: ta sama klasa), tier nie wyzszy, kultura wlasciciela magazynu (gracz: kultura gracza, AI: kultura partii); gdy w tej kulturze nic nie ma - bezkulturowy, potem ta sama strona Waskiego Morza (lista Essos z MountLaw); druga strona NIGDY. W obrebie tego: najwyzszy tier, potem suma pancerza, potem cena. Nie sa zamiennikami: unikaty, legendy (LegendaryLaw), korony, sprzet umarlych, NotMerchandise (rzadkosci lordow). Korony bez zamiennika znikaja. Gracz dostaje komunikat "X is not yours to keep - the quartermaster took Y in its place".
2. SWEEP przy kazdym wczytaniu: magazyn DTE gracza, tabor gracza, tabory AI, magazyny AI (`PartyArmories`) - kazda kopia -> zamiennik, liczba sztuk bez zmian (bez modyfikatora - modyfikator plyty na skorze bylby bez sensu).
3. BOHATEROWIE (gracz tez): unikat na kims, kto nie jest wlascicielem, schodzi na rzecz zamiennika w kulturze bohatera. Wlasciciel = item w jego szablonie XML (`BasicCharacterObject._equipmentRoster` przez refleksje; UWAGA: `CharacterObject.BattleEquipments` bohatera zwraca jego BIEZACY ekwipunek, nie szablon - sprawdzone w dekompilacji) albo nazwisko z listy Owners (odbicie Mends.NamesakeGear: Ramsay, Cersei, Stannis, Daenerys, Renly, Euron, Night King, Sandor, Brienne, Tyrion, Varys, Baelish, Gendry, Melisandre/Kinvara, Gregor).
...
**Mod:** CrashScribe | **Pliki:** `CrashScribe/src/Mends.cs` (UniqueIds, UniqueWard, RealmWard, WardReport)
**Zgloszenie (Jeff 16.09):** "biegaja z mieczami dotrakow, poustawiaj historyczne zaleznosci i zrob z tym porzadek".
**Przyczyna (dekompilacja DTE):** `PartyEquipmentDistributor.AssignEquipmentType` woli sprzet kultury zolnierza (`IsCultureCompatible`: `item.Culture == null || item.Culture == culture`), ale ma dwa wyjscia awaryjne: gdy w kulturze zolnierza nie ma nic - bierze cokolwiek, a gdy obca sztuka jest "duzo blizsza wzorca" (`num4 > num7 * 2`) - tez bierze obca. `arakh` to CraftedItem `culture=Culture.khuzait` (Dothraki): po jednej bitwie z Dothrakami lezy w magazynie i laduje na Riverlands Ranger.
**Zmiana:**
1. `RealmWard` - drugi postfix na `DoAssignAsync` (po SkillLawWard): sztuka z DRUGIEJ strony Waskiego Morza niz kultura zolnierza (Essos = lista MountLaw z 14.09 + steppe_bandits; za Murem liczy sie jako Westeros) schodzi z grzbietu, wraca na polke (`AddEquipmentToAssign`), a slot dostaje sztuke ze wzorca oddzialu (jak ArmourWard) - chyba ze wzorzec zolnierza SAM ja ma (najemnicy z Essos w Westeros nosza swoje). Bezkulturowe i `neutral_culture` przechodza. W obrebie jednej strony morza domy dziela sie lupem - tego nie tykamy (Polnoc w kolczudze z Dorzecza to zwykla wojna). Losowa pula DTE (FillEmptySlots) jest juz filtrowana po kulturze - tam nic nie trzeba.
2. `UniqueIds` - zestaw Gory (6 id) w `IsUniqueGear`; ta sama lista w Armoury.UniqueGear.Ids (AKTUALIZOWAC OBIE). Przez to kwatermistrz (BarredInBattle) i obie straze widza go jak pozostale unikaty.
